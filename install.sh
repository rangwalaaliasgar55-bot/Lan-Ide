#!/usr/bin/env bash
# Lan Ide — one-command install for Linux and macOS.
#
...
REPO_URL="${LAN_IDE_REPO_URL:-https://github.com/rangwalaaliasgar55-bot/Lan-Ide.git}"
# Optional release/branch pin. The downloadable Windows setup sets this to the
# release tag so users install the exact code that produced the EXE.
REPO_REF="${LAN_IDE_REF:-}"
DEST_DEFAULT="$HOME/lan-ide"
MIN_PY="3.11"
...
usage() {
  cat <<'EOF'
install.sh — install Lan Ide (Linux / macOS)

Usage:
  curl -fsSL https://raw.githubusercontent.com/rangwalaaliasgar55-bot/Lan-Ide/main/install.sh | bash
  ./install.sh
  ./install.sh --dir ~/lan-ide --no-start

Windows: in PowerShell
  irm https://raw.githubusercontent.com/rangwalaaliasgar55-bot/Lan-Ide/main/install.ps1 | iex

Options:
  --dir PATH     Clone / use this directory (default: ~/lan-ide)
...
if [[ -d "$DEST/plotspace" && -f "$DEST/plotspace/main.py" ]]; then
  ...
  if [[ -d "$DEST" && -n "$(ls -A "$DEST" 2>/dev/null || true)" ]]; then
    die "$DEST exists and is not a Lan Ide checkout. Pass --dir or move it aside."
  fi
  if [[ -n "$REPO_REF" ]]; then
    log "Cloning $REPO_URL ($REPO_REF) → $DEST"
    run git clone --depth 1 --branch "$REPO_REF" "$REPO_URL" "$DEST"
  else
...
# ── ~/.local/bin/lanide → venv entry point (NOT repo bin/lanide) ─────────
WRAP="$HOME/.local/bin/lanide"
log "Linking $WRAP → $DEST/venv/bin/lanide"
if [[ "$DRY_RUN" -eq 1 ]]; then
  ...
  cat > "$WRAP" <<EOF
#!/usr/bin/env bash
# Installed by Lan Ide install.sh — runs the app (plotspace.cli).
exec "$DEST/venv/bin/lanide" "\$@"
EOF
  chmod +x "$WRAP"
fi

if [[ ":$PATH:" != *":$HOME/.local/bin:"* ]]; then
  _line='export PATH="$HOME/.local/bin:$PATH"  # lan-ide'
  for rc in "$HOME/.bashrc" "$HOME/.zshrc"; do
    if [[ "$DRY_RUN" -eq 1 ]]; then
      log "[dry-run] ensure PATH in $rc"
      continue
    fi
    if [[ -f "$rc" ]] && grep -q 'lan-ide' "$rc" 2>/dev/null; then
      continue
    fi
...
if [[ "$NO_START" -eq 1 ]]; then
  log "Installed. Start with:  lanide"
...
if curl -s -o /dev/null --max-time 2 http://127.0.0.1:3000/api/health; then
  ...
  if en_wsl && [[ -x "$DEST/scripts/reiniciar-server.sh" ]]; then
    ...
    if command -v setsid >/dev/null 2>&1; then
      setsid nohup "$DEST/venv/bin/lanide" --sin-browser >>"$DEST/data/uvicorn.log" 2>&1 </dev/null &
    else
      nohup "$DEST/venv/bin/lanide" --sin-browser >>"$DEST/data/uvicorn.log" 2>&1 </dev/null &
    fi
...
if curl -s -o /dev/null --max-time 2 http://127.0.0.1:3000/api/health; then
  ...
  log "Installed, but the server did not answer yet. Start it with:  lanide"
