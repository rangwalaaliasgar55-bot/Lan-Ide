...
function Install-DesktopLauncher {
    ...
    if ($text -notmatch 'localhost:3000') {
        throw 'The launcher copied from WSL does not look like LanIde.bat.'
    }
...
if ($env:LAN_IDE_INSTALL_DRY_RUN -eq '1') { $dry = $true }

Write-Step 'Lan Ide for Windows (engine in WSL2).'
