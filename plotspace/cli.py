"""El comando `lanide`: levanta el motor y abre la app.

    lanide                  # levanta y abre el browser
    lanide --puerto 3100    # otro puerto
    lanide --sin-browser    # para correrlo en un servidor
"""
import argparse
import os
import sys
...
def construir_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument('--host', default=os.environ.get('LAN_IDE_HOST', '127.0.0.1'),
    ...
    p.add_argument('--datos', default=os.environ.get('LAN_IDE_DATA_DIR', ''),
...
def main(argv=None) -> int:
    ...
    if args.datos:
        os.environ['LAN_IDE_DATA_DIR'] = os.path.abspath(os.path.expanduser(args.datos))
    os.environ.setdefault('LAN_IDE_PORT', str(args.puerto))
    ...
    if args.host not in ('127.0.0.1', 'localhost', '::1'):
        print(f'[lanide] escuchando en {args.host}: la app queda accesible '
    ...
    if not args.sin_browser:
        threading.Thread(target=_abrir_cuando_responda, args=(url,),
                         daemon=True, name='lanide-browser').start()
