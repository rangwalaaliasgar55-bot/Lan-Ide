...
async def _startup():
    ...
    asyncio.get_event_loop().set_default_executor(
        ThreadPoolExecutor(max_workers=32, thread_name_prefix='lanide')
    )
...
#   embeber LanIde en un iframe (anti-clickjacking del workspace). No afecta a
#   LanIde embebiendo OTROS sitios (eso lo gobiernan los headers del hijo).
# - Referrer-Policy no-referrer: ninguna URL de LanIde viaja en el header
...
async def _middleware_http(request: Request, call_next):
    ...
    if not lanide_auth.host_permitido(request.headers.get('host'), lanide_auth.hosts_extra()):
        ...
        if (path.startswith('/api/')
                ...
                and not lanide_auth.origen_permitido(request.headers.get('origin'),
...
async def manifest():
    # Web App Manifest servido desde la RAÍZ (scope "/") para que LanIde sea
