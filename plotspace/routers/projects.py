...
# Raíz del propio repo LanIde (este archivo vive en plotspace/routers/ → ../../ = raíz).
_REPO_ROOT = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..'))


def _es_ruta_protegida(ruta_abs: str) -> bool:
    """True si `ruta_abs` NO puede borrarse ni renombrarse: rutas del sistema,
    el HOME, o CUALQUIER parte del árbol del propio LanIde (la raíz, un ancestro
    suyo, o un subdirectorio interno).

    El blindaje del árbol de LanIde evita que el dashboard se destruya a sí mismo
    ...
    /home/user/lanide se llevaron el repo entero. Falla cerrado."""
    if not ruta_abs:
        return False
...
async def eliminar_proyecto(
    ...
    Orden importante (para que el rmtree no falle en silencio):
      ...
      2. Matar TODAS las sesiones tmux lanide_{id} (sino mantienen file handles
    ...
    if keep_folder:
        ...
        folder_error = f"No borré '{ruta_abs}': es el propio LanIde o una ruta protegida. Proyecto sí quitado del workspace."
    else:
