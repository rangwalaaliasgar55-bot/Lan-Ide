"""
LAN_IDE — Sistema: versión + reinicio in-app.
...
commitea lo suyo y se puede aplicar lo terminado, aunque otra terminal de LanIde
siga trabajando (pedido del usuario 2026-06-18). Como el HEAD es el de ESTE repo,
solo cuenta el código de LanIde. Las novedades salen de los mensajes de commit.
...
# del árbol —.lan-ide/LIVE.md, MAILBOX, data/, .workspace/, docs— la app lo reescribe
...
def _estado_git():
    ...
    tarea se commiteó, así se aplica lo terminado aunque otra terminal de LanIde
    siga trabajando (pedido del usuario 2026-06-18). Como el HEAD es el de ESTE
    repo, solo cuenta el código de LanIde (commits de otros proyectos no lo mueven).
...
# Este flag queda como dato (¿hay laburo en curso en LanIde?) por si lo quiere
# el modal/tooltip; se calcula SOLO sobre el proyecto de LanIde (este repo):
...
def _lanide_project_id():
    """project_id del proyecto cuyo ruta es ESTE repo (donde corre el server). El
    código de LanIde se trabaja solo en ese proyecto; los agentes de OTROS
...
def _agentes_trabajando() -> bool:
    # SOLO el proyecto de LanIde (este repo): un agente trabajando en OTRO
    # proyecto no toca este código → no bloquea el "Actualizar ahora". Si no
    # ubicamos el proyecto de LanIde, fallback global (conservador, como antes).
    pid = _lanide_project_id()
...
# Discord el lanzador LanIde.exe (scripts/lanide-shell.cs — el pipe
...
def formatear_seccion(seccion, locale) -> str:
    ...
    (o None, ej. recién booteado) cae a LanIde."""
    es, en = _PRESENCE_SECCIONES.get(seccion, ('LanIde', 'LanIde'))
...
def reiniciar_servidor(retraso: float = 1.0):
    ...
    threading.Thread(target=_demorado, daemon=True, name='lanide-reexec').start()
...
async def presence_state(payload: dict):
    ...
    (in-memory). Lo consume LanIde.exe para la Rich Presence de Discord.
...
@router.get("/presence")
async def presence():
    """Contenido de la tarjeta 'Jugando LanIde' de Discord. Lo poletea LanIde.exe
