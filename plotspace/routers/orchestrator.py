...
SYSTEM_PROMPT = """Sos LAN_IDE, orquestador de agentes de IA para desarrollo de software.
...
Tiene que ser AUTOSUFICIENTE y debe incluir, en este orden:
  ...
  5. PREVIEW — SOLO si el resultado es algo visual (página web, app, UI):
     ...
     foreground, que te bloquea) y dejalo corriendo: LanIde detecta la
     URL y se la muestra al usuario automáticamente. REGLA DE PUERTOS:
     el 3000 está PROHIBIDO (ahí corre Lan Ide); antes de
...
Estilo LanIde (Iron Man): conciso, directo, técnico, sin relleno.
...
Tu lectura es:
  ...
  • Sabés si hay un preview server corriendo (si el workflow incluyó
    frontend, LAN_IDE ya lo lanzó automáticamente en localhost:8080+).
...
Cuando un workflow termina e incluyó frontend:
  • El backend de LAN_IDE YA lanza un http.server automáticamente.
...
vez, siempre). Sus campos: 'message' (texto al usuario, tono LanIde, va
...
actions[].type válidos (NADA MÁS):
  ...
  "stop_preview"   — sin args        → apaga el servidor http.server que LAN_IDE
  ...
  • "cerrá el servidor" / "apagá el preview" / "matá el puerto 8081" /
    "bajá el servidor" / "cerrá la pestaña del preview"
      → stop_preview (NO close_terminal — el preview NO corre en una
        terminal de LAN_IDE, corre como proceso aparte iniciado al hacer merge)
...
ese puerto no lo lanzó LAN_IDE.
...
SCOUT (opcional, recomendado para objetivos amplios o codebase desconocida):
  • Un paso_0 con rol "scout" y archivos []: explora el código relevante
    ...
    .lan-ide/memory/ (formato del protocolo del CLAUDE.md del proyecto),
    terminando con un resumen de qué memorias creó.
  • Los builders dependen del scout (depende_de: "paso_0") y su tarea les
    indica leer .lan-ide/memory/INDEX.md antes de empezar.
...
'message' primero en tono LanIde. Antes de llamarla, revisá mentalmente:
tono LanIde, archivos disjuntos y sacados del mapa real, terminales libres
...
RESPONDER_TOOL = {
    ...
    "description": (
        ...
        "'message' es el texto en tono LanIde que ve el usuario y va PRIMERO. "
    ...
    "input_schema": {
        ...
        "properties": {
            "message": {
                "type": "string",
                "description": "Texto al usuario, tono LanIde (conciso, directo, sin relleno). Va PRIMERO. Se streamea token a token.",
            },
            "actions": {
                "type": "array",
...
def _extraer_message_parcial(texto: str) -> str:
    ...
    # del valor de 'message' (LanIde explicando código) y NO debe tocarse — strippearlo descartaría
...
async def _procesar_respuesta_orquestador(raw_text, usage, project, req, terminals_activas, stop_reason=None):
    ...
    # NO strippear fences acá: si 'message' contiene un ``` de código (LanIde explicándole
    ...
    lanide_message, actions, workflow_data = _sanitizar_respuesta(parsed, raw_text)
    ...
    if stop_reason == "max_tokens" and workflow_data:
        workflow_data = None
        lanide_message = (lanide_message + " (me corté por longitud, señor — repetime la orden más acotada).").strip()
    ...
    for action in actions:
        ...
        if atype == "spawn_terminal":
            ...
            if trabajando:
                ...
                lanide_message = (lanide_message + " ⚠️ No cerré nada: " +
                                  ", ".join(trabajando) +
            ...
            if tid:
                ...
                if motivo:
                    lanide_message = (lanide_message + f" ⚠️ Ojo: {motivo}.").strip()
...
texto alrededor — con: 'message' (texto al usuario, tono LanIde, va
PRIMERO), 'actions' (array; usá [{"type":"none"}] cuando solo hay workflow
o respuesta conversacional) y 'workflow' (opcional, solo si la tarea es
...
'message' primero en tono LanIde."""
...
def _guard_api_key() -> str:
    ...
    para que el frontend lo muestre como empty-state limpio. El chat de LanIde (y el
    ...
    if not api_key:
        raise HTTPException(status_code=409, detail={
            ...
            "message": "Configurá ANTHROPIC_API_KEY (plotspace/.env) para usar el chat "
                       "de LanIde. Los agentes en terminales (BYOK) no la necesitan.",
        })
    return api_key
...
def _bloque_memoria_para_orden(project_ruta: str, mensaje: str) -> str:
    ...
    for m in rel:
        ...
        lineas.append(f"  • {m['titulo']}{marca} — .lan-ide/memory/{m['slug']}.md")
    return '\n'.join(lineas)
...
def _guardar_imagen_temporal(image_base64: str, media_type: str = None):
    ...
    try:
        ...
        fd, ruta = tempfile.mkstemp(prefix='lanide-orq-img-', suffix=ext)
...
# Puerto real de LanIde (modo app: el shell lo elige dinámico vía LAN_IDE_PORT)
_LAN_IDE_PORT = os.environ.get('LAN_IDE_PORT', '3000')


def _fuente_permite_lanide(fuente: str) -> bool:
    """¿Una fuente de CSP `frame-ancestors` permite embeber a LanIde
    ...
    que NO es el 3000 de LanIde → no cuenta."""
    ...
    if host != 'localhost':
        return False
    return bool(sep) and port in ('*', _LAN_IDE_PORT)
...
async def listar_servers_preview(project_id: int):
    ...
    (el http.server propio de LanIde + cada dev server de agente detectado, con
    ...
    if entry:
        ...
        if proc.poll() is None:
            servers.append({'url': url, 'port': urlparse(url).port,
                            'terminal_id': None, 'terminal_nombre': 'LanIde', 'propio': True})
        else:
...
async def detener_preview(project_id: int, body: Optional[StopPreviewBody] = None):
    ...
    mata por el puerto. NUNCA toca el 3000 (LanIde) — guard en matar_puerto.
    Un DEMO del propio LanIde (:3000/static/<dir>/…) no tiene proceso: acá solo
    se OCULTA del menú (descartar) sin tocar absolutamente nada del workspace."""
    from urllib.parse import urlparse
    from plotspace.core.dev_detect import descartar, es_demo_lanide
    ...
    if objetivo and es_demo_lanide(objetivo):
...
def _paso_reviewer(nombre: str, objetivo: str) -> dict:
    ...
    tarea = (
        ...
        "y con lo que dice .lan-ide/memory/INDEX.md?\n"
        ...
        "guardalo en .lan-ide/memory/ según el protocolo.\n\n"
...
async def ejecutar_workflow(workflow_data: dict, project_id: int, count_base: int) -> tuple:
    ...
    for i, paso in enumerate(pasos):
        ...
        print(f'[workflow] Sesión tmux lanide_{tid} lista (CLI={ia_type or "shell"}) en {project_path}')
...
async def _sesion_tmux_viva(terminal_id: int) -> bool:
    """True si la sesión tmux lanide_{terminal_id} existe."""
    try:
...
def _puertos_ocupados_por_lanide() -> set:
    """Puertos que LAN_IDE ya asignó a otros proyectos (aunque el bind real
...
def _puerto_libre(start: int, end: int, excluir: set = None) -> Optional[int]:
    ...
    LAN_IDE ya asignó a OTROS previews del registry — eso evita la race
    ...
    import socket
    ocupados_lanide = _puertos_ocupados_por_lanide()
    excluir = excluir or set()
    for p in range(start, end):
        if p in ocupados_lanide or p in excluir:
            continue
        try:
            s = socket.socket()
            s.bind(('127.0.0.1', p))
...
def _preview_url_activo(project_id: int) -> Optional[str]:
    ...
    Prioridad: http.server lanzado por LanIde → dev server de un agente
...
def _preview_urls_activas(project_id: int) -> list:
    ...
    pestañas: primero el http.server propio (si LanIde lanzó uno), después los
