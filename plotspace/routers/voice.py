...
def _nuevo_dir_audio() -> str:
    ...
    rutas fijas /tmp/lanide_audio.* que dos requests concurrentes se pisaban
    (devolvían audio cruzado) y que eran un nombre predecible en /tmp (CWE-377)."""
    return tempfile.mkdtemp(prefix="lanide_audio_")
...
_VOCABULARIO_DOMINIO = (
    "LanIde, Claude Code, Codex, Gemini, qwen, opencode, antigravity, tmux, "
...
async def _texto_a_base64(texto: str, voces: list[str] | None = None) -> str:
    ...
    fd, tmp_path = tempfile.mkstemp(suffix=".mp3", prefix="lanide_tts_")
    os.close(fd)
...
async def hablar(req: SpeakRequest):
    ...
    inglesa, así TODA la voz de LanIde sigue el idioma de la UI — el "de acuerdo
