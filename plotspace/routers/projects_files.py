...
# Archivos a ocultar (seguridad). Incluye los secretos del PROPIO LanIde por si
# su repo está registrado como proyecto navegable (el caso real que filtraba el
# token y la API key vía el editor).
HIDDEN_FILES = {
    '.env', '.env.local', '.env.production', '.env.development',
    'lanide_token.txt', 'telegram.json',
    'lanide.db', 'lanide.db-wal', 'lanide.db-shm',
}
...
# El _safe_join confina al proyecto, pero si el proyecto ES el repo de LanIde,
# data/lanide_token.txt y plotspace/.env quedan DENTRO del proyecto y se servirían.
...
_LAN_IDE_ROOT = os.path.realpath(os.path.join(os.path.dirname(__file__), '..', '..'))
_LAN_IDE_DATA = os.path.join(_LAN_IDE_ROOT, 'data')
_LAN_IDE_ENV  = os.path.join(_LAN_IDE_ROOT, 'backend', '.env')
# Con LAN_IDE_DATA_DIR los datos pueden vivir FUERA del repo: ese directorio
...
def _es_servible(full: str) -> bool:
    ...
    if real == _LAN_IDE_ENV or real == _LAN_IDE_DATA or real.startswith(_LAN_IDE_DATA + os.sep):
        return False
...
async def obtener_arbol(project_id: int):
    ...
    return {
        ...
        'children':  children,
        # True para el propio LanIde (/home/user/lanide): el front SOLO deja borrar
...
async def guardar_archivo(project_id: int, datos: FileSaveRequest):
    ...
    # No permitir leer NI sobrescribir secretos (token/API key/db del propio LanIde)
    _guard_servible(full)
    ...
    if era_nuevo:
        _marcar_creado(project_id, datos.path)   # creado desde la UI → borrable en el editor aun en LanIde
...
async def subir_archivos(
    ...
    for i, f in enumerate(files):
        ...
        try:
            ...
            _marcar_creado(project_id, rel_final)   # subido desde la UI → borrable en el editor (aun en LanIde)
        except Exception as e:
...
async def crear_directorio(project_id: int, datos: MkdirRequest):
    ...
    _marcar_creado(project_id, rel)   # creada desde la UI → borrable en el editor aun en LanIde
...
async def borrar_directorio(project_id: int, path: str = Query("")):
    ...
    # Blindaje anti auto-destrucción: en el árbol del propio LanIde SOLO se borra lo
...
async def borrar_archivo(project_id: int, path: str = Query(...)):
    ...
    # Blindaje anti auto-destrucción: en LanIde SOLO se borra lo creado desde el editor.
    from plotspace.routers.projects import _es_ruta_protegida
...
def _buscar_python(base: str, patron, q_raw: str) -> dict:
    ...
    for dirpath, dirnames, filenames in os.walk(base):
        ...
        # Además podar el dir de secretos del propio LanIde (data/) por ruta real.
        dirnames[:] = [
            d for d in dirnames
