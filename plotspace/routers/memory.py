# LAN_IDE — Memoria compartida del proyecto.
# Grafo de conocimiento en `.lan-ide/memory/` del proyecto: archivos markdown
...
# inyectado en el CLAUDE.md del proyecto (markers LAN_IDE_MEMORY_*) les dice
...
MEM_DIRNAME = os.path.join('.lan-ide', 'memory')

_WIKILINK_RE   = re.compile(r'\[\[([^\]\n]+)\]\]')
_FRONT_RE      = re.compile(r'^---\s*\n(.*?)\n---\s*\n?', re.DOTALL)
_SLUG_LIMPIO   = re.compile(r'[^a-z0-9]+')

PROTOCOLO_MARKER_START = '<!-- LAN_IDE_MEMORY_START -->'
PROTOCOLO_MARKER_END   = '<!-- LAN_IDE_MEMORY_END -->'
...
def _regenerar_index(project_path: str, memorias: Optional[list] = None):
    ...
    lineas = [
        ...
        '<!-- Autogenerado por LanIde: agrupado por categoría, una línea por memoria. -->\n',
    ]
...
def _inyectar_protocolo_en(archivo_md: str):
    """Inserta/refresca el bloque del protocolo (markers LAN_IDE_MEMORY_*) en un
...
def asegurar_memoria_proyecto(project_path: str):
    """Hook para _preparar_proyecto (terminals.py): crea .lan-ide/memory/ con
    ...
    # sin pisar: en el propio LanIde esos slugs ya existen, no duplica.
    try:
