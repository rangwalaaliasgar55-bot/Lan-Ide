...
_EXCLUIR_DIRS = {'node_modules', '.git', '.expo', '.expo-shared', 'ios', 'android',
                 'build', 'dist', '.next', 'coverage', '__tests__', '.lan-ide',
...
# servicio lanide_mpreview_* → dos Metro sobre la misma carpeta + consola fuera
...
# app pinta. Este endpoint sirve el HTML de la app DESDE el origen de LanIde con
...
def _url_sampler_valida(url: str) -> bool:
    """SOLO el origen (sin path) de un dev server local; nunca LanIde (:3000)
...
def _inyectar_sampler(html: str, base_url: str) -> str:
    ...
    relativo debe resolver contra LanIde) y DESPUÉS el <base> hacia Metro (que
...
async def asset_proxy(project_id: int, u: str):
    ...
    (bug real 2026-07-11, Outfit de @expo-google-fonts). Servidas desde LanIde
...
def _puertos_candidatos(texto: str) -> list:
    ...
    sacarlo de ahí. Excluye PUERTOS_IGNORADOS (LanIde :3000, inspector ngrok :4040)."""
...
# pendientes. Viven en data/lanide.db (local, gitignoreado) — JAMÁS en el repo,
