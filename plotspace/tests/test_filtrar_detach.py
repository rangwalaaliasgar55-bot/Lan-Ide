...
def test_quita_linea_detach_sola():
    assert _filtrar_detach('[detached (from session lanide_510)]\r\n') == ''


def test_quita_detach_sin_newline():
    assert _filtrar_detach('[detached (from session lanide_42)]') == ''


def test_quita_detach_con_cr_previo():
    # tmux suele anteponer un \r
    assert _filtrar_detach('\r[detached (from session lanide_7)]\r\n') == ''


def test_conserva_lo_de_alrededor():
    txt = 'foo\n[detached (from session lanide_1)]\r\nbar\n'
...
def test_multiples_detach_en_un_chunk():
    txt = ('[detached (from session lanide_510)]\r\n'
           '[detached (from session lanide_510)]\r\n')
