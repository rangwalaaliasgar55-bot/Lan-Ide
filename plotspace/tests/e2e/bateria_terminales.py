...
Uso (contra un server EFÍMERO aislado, jamás contra el :3000 del usuario):
  ...
  3. LAN_IDE_QA_DIR=<WT> venv/bin/python plotspace/tests/e2e/bateria_terminales.py
  4. Limpiar: matar el server, tmux kill-session de las lanide_<n> de QA, restaurar <WT>.
...
BASE = os.environ.get('LAN_IDE_QA_BASE', 'http://127.0.0.1:5731')
WT = os.environ.get('LAN_IDE_QA_DIR', '/home/user/lanide/.claude/worktrees/terminales-un-emulador')
...
async def convergencia(page, tids):
    ...
    for t in tids:
        ...
        win = tmuxq(f'lanide_{t}', '#{window_width}x#{window_height}')
...
async def main():
    ...
    async with async_playwright() as p:
        ...
        await page.evaluate(f'''() => localStorage.setItem('lanide.terminals.layout.{pid}',
