...
(el tty), NO una sesión. El backend llamaba `refresh-client -t lanide_<id>`
...
def test_refresh_apunta_a_cada_tty_no_a_la_sesion():
    refreshes = _correr('lanide_42', '/dev/pts/14\n/dev/pts/20\n')
    ...
    assert not any('lanide_42' in r for r in refreshes)


def test_refresh_sin_clientes_no_rompe():
    refreshes = _correr('lanide_42', '')   # sesión sin clientes attachados
    assert refreshes == []


def test_refresh_un_solo_cliente():
    refreshes = _correr('lanide_99', '/dev/pts/3\n')
