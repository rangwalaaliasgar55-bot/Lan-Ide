...
- El pip es el requirements.txt COMPLETO (voz local, TTS, todo). No hay
  instalador "diet" que recorte funciones de LanIde.
- El comando que queda en PATH es el `lanide` del venv (el motor), no bin/lanide
...
def test_install_sh_deja_lanide_del_venv_en_path():
    """bin/lanide del repo es OTRO comando (abre un proyecto). El wrapper
    tiene que apuntar al entry point del venv."""
    src = _leer(_SH)
    assert '.local/bin' in src
    assert 'venv/bin/lanide' in src
...
def test_install_ps1_instala_wsl_y_deja_acceso_en_el_escritorio():
    ...
    assert 'abrir-lanide-app.bat' in src
...
def test_readme_tiene_un_comando_por_sistema():
    ...
    assert 'rangwalaaliasgar55-bot/Lan-Ide' in readme
