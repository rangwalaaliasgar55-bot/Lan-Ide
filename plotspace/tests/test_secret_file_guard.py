...
Cuando el repo de LanIde está registrado como proyecto navegable, su
data/lanide_token.txt y plotspace/.env quedaban DENTRO del proyecto y el editor
...
def _proyecto_con(archivos: dict):
    ...
    fresh_db()
    d = tempfile.mkdtemp(prefix="lanide_secguard_")
...
def test_token_por_nombre_no_se_lee():
    client, pid = _proyecto_con({'data/lanide_token.txt': 'SECRETO-123', 'ok.txt': 'hola'})
    assert client.get(f'/api/projects/{pid}/files/read?path=data/lanide_token.txt').status_code == 403
    assert client.get(f'/api/projects/{pid}/files/raw?path=data/lanide_token.txt').status_code == 403
    assert client.get(f'/api/projects/{pid}/files/stat?path=data/lanide_token.txt').status_code == 403
...
def test_busqueda_no_devuelve_el_token():
    client, pid = _proyecto_con({
        'data/lanide_token.txt': 'TOKENSECRETO',
    ...
    assert 'data/lanide_token.txt' not in rutas
...
def test_repo_de_lanide_como_proyecto_no_filtra_token():
    # El caso EXACTO de la auditoría: proyecto cuyo dir ES el repo de LanIde.
    fresh_db()
    raiz = pf._LAN_IDE_ROOT
    if not os.path.isfile(os.path.join(raiz, 'data', 'lanide_token.txt')):
...
def test_save_no_sobreescribe_secretos():
    client, pid = _proyecto_con({'data/lanide_token.txt': 'ORIGINAL'})
    r = client.post(f'/api/projects/{pid}/files/save',
                    json={'path': 'data/lanide_token.txt', 'content': 'PWNED'})
...
def test_rename_no_exfiltra_secreto_por_nombre():
    ...
    client, pid = _proyecto_con({'data/lanide_token.txt': 'SECRETO'})
    r = client.post(f'/api/projects/{pid}/files/rename',
                    json={'src': 'data/lanide_token.txt', 'dst': 'robado.txt'})
    assert r.status_code == 403
...
def test_delete_secreto_bloqueado():
    client, pid = _proyecto_con({'data/lanide_token.txt': 'x'})
    r = client.request('DELETE', f'/api/projects/{pid}/files',
                       params={'path': 'data/lanide_token.txt'})
    assert r.status_code == 403


def test_invariante_es_servible():
    # El invariante que usan TODOS los endpoints (read/save/upload/rename/delete):
    # bloquea data/ y plotspace/.env del repo de LanIde por realpath; permite el
    # data/ de CUALQUIER OTRO proyecto.
    assert pf._es_servible(os.path.join(pf._LAN_IDE_DATA, 'x.txt')) is False
    assert pf._es_servible(os.path.join(pf._LAN_IDE_DATA, 'browser_profile', 'Default', 'Cookies')) is False
    assert pf._es_servible(pf._LAN_IDE_ENV) is False
