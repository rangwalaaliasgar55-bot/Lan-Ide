...
(Anthropic, MCP, lo que sea) ni el token de LanIde — eso cuesta plata. El
...
2. Detecta los VALORES REALES de los secretos locales (data/lanide_token.txt,
...
def test_detecta_valor_real_local():
    ...
    hallazgos = scan.encontrar_secretos(
        f'print("{valor}")', valores=[('token-lanide', valor)])
    assert hallazgos
    assert any(h['patron'] == 'token-lanide' for h in hallazgos)


def test_valores_locales_lee_archivos(tmp_path, monkeypatch):
    (tmp_path / 'data').mkdir()
    (tmp_path / 'data' / 'lanide_token.txt').write_text('tokendeprueba123456\n')
...
def test_no_flaggea_codigo_normal():
    limpio = '\n'.join([
        ...
        'token = data["lanide_token"]',          # referencia, no literal
        'const x = "data/lanide_token.txt"',     # ruta, no valor
