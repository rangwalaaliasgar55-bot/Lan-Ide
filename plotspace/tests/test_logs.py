...
AISLADO del data/lanide.log real: antes cada corrida de suite appendeaba
...
@contextmanager
def _log_aislado():
    fd, path = tempfile.mkstemp(suffix='.log', prefix='lanide_test_')
    os.close(fd)
    orig = logs._LOG_PATH
    logs._LOG_PATH = path
    try:
...
def test_no_toca_el_log_real():
    ...
    ni engordar el data/lanide.log del producto."""
    real = logs._LOG_PATH
    ...
    assert tam_antes == tam_despues, "el test ensució el lanide.log real"
