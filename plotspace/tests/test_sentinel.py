...
def test_ruta_sentinel():
    ...
    assert r == os.path.join('/home/user/proj', '.lan-ide', 'signals', 'terminal_7.json')
...
def test_lee_y_borra_one_shot():
    with tempfile.TemporaryDirectory() as d:
        sig_dir = os.path.join(d, '.lan-ide', 'signals')
        os.makedirs(sig_dir)
...
def test_ciclo_procesa_sentinel_de_paso_running():
    ...
    with tempfile.TemporaryDirectory() as d:
        sig = os.path.join(d, '.lan-ide', 'signals')
        os.makedirs(sig)
