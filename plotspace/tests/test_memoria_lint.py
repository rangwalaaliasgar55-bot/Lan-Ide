"""
Test: memoria_lint — chequeos deterministas de salud de .lan-ide/memory/.
...
def test_mantener_memorias_regenera_index():
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        idx = open(os.path.join(d, '.lan-ide', 'memory', 'INDEX.md')).read()
...
def test_auto_archivar_solo_viejas_sin_uso():
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        src = open(os.path.join(d, '.lan-ide', 'memory', 'anciana-olvidada.md')).read()
        assert 'estado: archivo' in src and 'archivado-auto: 2026-07-19' in src
        # las demás intactas
        assert 'estado: vigente' in open(os.path.join(d, '.lan-ide', 'memory', 'fresca.md')).read()
        assert 'estado: lapida' in open(os.path.join(d, '.lan-ide', 'memory', 'lapida-vieja.md')).read()
