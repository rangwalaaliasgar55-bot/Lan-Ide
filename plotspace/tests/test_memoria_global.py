...
Cada proyecto nuevo del workspace arranca con .lan-ide/memory/ VACÍA — enjambre
...
def test_promover_agrega_al_store_global():
    with tempfile.TemporaryDirectory() as store, tempfile.TemporaryDirectory() as proj:
        mdir = os.path.join(proj, '.lan-ide', 'memory')
        os.makedirs(mdir)
        ...
        with tempfile.TemporaryDirectory() as nuevo:
            os.makedirs(os.path.join(nuevo, '.lan-ide', 'memory'))
            mg.sembrar(nuevo, store_dir=store)
            assert os.path.exists(os.path.join(nuevo, '.lan-ide', 'memory', 'leccion-x.md'))
