...
configuración (incluido el SessionStart hook que LanIde ya instalaba) y un
...
def test_instala_ambos_eventos(tmp_path):
    ...
    assert asegurar_hooks_provenance(str(p), 'python3 /r/scripts/lanide_ops_hook.py') is True
    d = _leer(p)
...
def test_es_idempotente(tmp_path):
    p = tmp_path / 'settings.json'
    cmd = 'python3 /r/scripts/lanide_ops_hook.py'
...
def test_preserva_configuracion_existente(tmp_path):
    """El settings del usuario tiene el SessionStart de LanIde y lo suyo: nada
    ...
    original = {
        ...
        'hooks': {'SessionStart': [{'hooks': [
            {'type': 'command', 'command': 'python3 /r/scripts/lanide_claude_hook.py'}]}]},
    }
    p.write_text(json.dumps(original), encoding='utf-8')
    asegurar_hooks_provenance(str(p), 'python3 /r/scripts/lanide_ops_hook.py')
    d = _leer(p)
    assert d['model'] == 'opus'
    assert d['permissions']['allow'] == ['Bash(ls)']
    assert d['hooks']['SessionStart'][0]['hooks'][0]['command'].endswith('lanide_claude_hook.py')
...
def test_tiene_timeout_corto(tmp_path):
    """Sin timeout, un LanIde colgado congelaría la herramienta del agente."""
    p = tmp_path / 'settings.json'
    asegurar_hooks_provenance(str(p), f'python3 /r/{NOMBRE_SCRIPT}')
    d = _leer(p)
...
def test_el_session_hook_usa_el_mismo_trato_que_el_de_provenance(tmp_path):
    ...
    file 'C:\\proyectos\\lanide\\UsersUSER…'".
...
def test_el_session_hook_apunta_a_su_script(tmp_path):
    for sistema in ('nt', 'posix'):
        ...
        assert 'lanide_claude_hook.py' in cmd, cmd
