...
def test_pegar_usa_el_buffer_de_tmux():
    cmds = comandos_pegar_tarea('lanide_7', 'hola')
...
def test_pegar_usa_doble_guion_antes_del_texto():
    ...
    cmds = comandos_pegar_tarea('lanide_7', '-rf algo')
    assert cmds[0][-2] == '--'


def test_pegar_apunta_a_la_sesion_correcta():
    cmds = comandos_pegar_tarea('lanide_42', 'x')
    assert '-t' in cmds[1] and 'lanide_42' in cmds[1]


def test_cada_terminal_usa_su_propio_buffer():
    """Dos agentes recibiendo tarea a la vez no se pisan el buffer."""
    a = comandos_pegar_tarea('lanide_1', 'x')[0][3]
    b = comandos_pegar_tarea('lanide_2', 'x')[0][3]
    assert a != b
