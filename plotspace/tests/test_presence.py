...
La tarjeta "Jugando LanIde" de Discord la ARMA este backend (GET
/api/system/presence: details/state ya formateados, bilingüe ES/EN + conteo de
agentes vivos) y la EMPUJA al IPC de Discord el lanzador LanIde.exe
(scripts/lanide-shell.cs — el pipe discord-ipc-N vive en Windows y WSL no lo
alcanza).
...
def test_formatear_seccion_desconocida_cae_a_lanide():
    # None (recién booteado, sin reporte) o clave inexistente → LanIde.
    assert system.formatear_seccion(None, 'en') == 'In LanIde'
    assert system.formatear_seccion('inexistente', 'es') == 'En LanIde'
