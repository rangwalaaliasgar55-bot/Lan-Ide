"""Tests del comando `lanide` (`plotspace/cli.py`).
...
def test_el_comando_se_llama_lanide():
    assert cli.construir_parser().prog == 'lanide'
...
def test_el_puerto_sale_del_entorno_si_esta(monkeypatch):
    ...
    monkeypatch.setenv('LAN_IDE_PORT', '4321')
    import importlib
    importlib.reload(cli)
    assert cli.construir_parser().parse_args([]).puerto == 4321
    monkeypatch.delenv('LAN_IDE_PORT')
    importlib.reload(cli)
