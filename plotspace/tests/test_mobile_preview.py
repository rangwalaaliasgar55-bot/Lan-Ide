...
def test_escanear_panes_ignora_lanide_3000():
    # El :3000 (LanIde) lo excluye extraer_urls_locales → el probe nunca se llama.
    llamadas = []
...
def test_puertos_excluye_lanide_3000():
...
def test_sampler_url_valida_solo_origen_local():
    ...
    # puertos vedados: LanIde y el inspector de ngrok
...
def test_sampler_inyecta_script_antes_del_base():
    ...
    # debe resolver contra LanIde, no contra Metro)
...
def test_asset_url_valida_solo_dev_servers_locales():
    ...
    # vedados / inválidos
    assert _url_asset_valida('http://localhost:3000/etc') is None      # LanIde
