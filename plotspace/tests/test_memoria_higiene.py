"""
Test: higiene del sistema de memoria compartida (.lan-ide/memory/).
...
def test_regenerar_no_escribe_si_no_cambio():
    with tempfile.TemporaryDirectory() as d:
        ...
        mem._regenerar_index(d)
        path = os.path.join(d, '.lan-ide', 'memory', 'INDEX.md')
...
def test_put_bumpea_actualizado_en_disco():
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        src = open(os.path.join(d, '.lan-ide', 'memory', 'para-editar.md')).read()
