...
Cubre el des-embebido de `qa-browser-lanide`: la skill es larga (~75 líneas)
...
def _setup_proyecto(tmp_path):
    ...
    with open(os.path.join(skills_dir, 'qa-browser-lanide.md'), 'w', encoding='utf-8') as f:
        f.write(CUERPO_QA)
...
def test_qa_browser_se_des_embebe_a_puntero(tmp_path):
    ...
    assert 'Skill qa-browser-lanide — ver `.claude/skills/qa-browser-lanide.md`' in md
    # ...y el cuerpo completo de la skill NO (ni su frontmatter ni el relleno)
    assert 'name: qa-browser-lanide' not in md
