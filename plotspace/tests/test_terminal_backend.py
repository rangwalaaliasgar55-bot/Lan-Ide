...
def test_nombre_de_sesion_es_el_historico():
    ...
    assert TmuxBackend().nombre_sesion(441) == 'lanide_441'
...
def test_crear_arma_el_argv_completo(monkeypatch, tmp_path):
    ...
    ok = TmuxBackend().crear(EspecSesion(
        ...
        env={'LAN_IDE_TERMINAL_ID': '7', 'CODEX_HOME': '/tmp/codex'},
        cols=220, rows=50,
    ))
    assert ok is True
    ...
    assert argv[:5] == ['tmux', 'new-session', '-d', '-s', 'lanide_7']
    ...
    assert argv[argv.index('-e') + 1] == 'LAN_IDE_TERMINAL_ID=7'
...
def test_matar_usa_target_exacto(monkeypatch):
    ...
    # una lanide_1 ya muerta se llevaba puesta a lanide_12, viva y trabajando.
    import asyncio
    ...
    assert '=lanide_1' in argv, f'target sin "=": {argv}'
...
def test_enviar_texto_es_literal(monkeypatch):
    ...
    assert argv == ['tmux', 'send-keys', '-t', 'lanide_3', '-l', '--', '--peligroso Enter']
...
def test_enviar_tecla_no_es_literal(monkeypatch):
    ...
    assert f.argv_con('send-keys') == ['tmux', 'send-keys', '-t', 'lanide_3', 'Enter']
...
def test_capturar_pantalla_y_scrollback(monkeypatch):
    ...
    assert f.argv_con('capture-pane') == ['tmux', 'capture-pane', '-t', 'lanide_4', '-p']
...
def test_titulos_vivos_mapea_sesion_a_titulo(monkeypatch):
    ...
    _fake(monkeypatch, default=(0, 'lanide_1\tescribiendo tests\nlanide_2\tidle\nbasura\n'))
    titulos = TmuxBackend().titulos_vivos()
    assert titulos == {'lanide_1': 'escribiendo tests', 'lanide_2': 'idle'}


def test_listar_sesiones(monkeypatch):
    _fake(monkeypatch, default=(0, 'lanide_1\nlanide_2\notra\n'))
    assert TmuxBackend().listar_sesiones() == {'lanide_1', 'lanide_2', 'otra'}
...
def test_refrescar_apunta_al_tty_no_a_la_sesion(monkeypatch):
    ...
    assert not f.hubo('refresh-client', 'lanide_6'), 'refrescó la sesión en vez del tty'
