"""
Test: la status bar de tmux (la barra verde de abajo «[lanide_<id>:claude] …
...
def test_reconciliar_apaga_status_en_sesiones_vivas():
    ...
    def _fake_run(args, **kw):
        ...
        if "list-sessions" in a:
            return mock.Mock(returncode=0, stdout="lanide_790\nlanide_791\notra_x\n", stderr="")
    ...
    calls = _run_calls(m)
    assert _setea_status_off(calls, "lanide_790"), \
        f"no se apagó la status bar en lanide_790. Calls: {calls}"
    assert _setea_status_off(calls, "lanide_791"), \
        f"no se apagó la status bar en lanide_791. Calls: {calls}"
    # Sesiones que no son de lanide NO se tocan.
