"""Backup automático de data/lanide.db (post-incidente 2026-07-03).

El rmtree del incidente se llevó data/lanide.db y fue lo ÚNICO irrecuperable:
...
def _db_de_prueba(dir_):
    ...
    path = os.path.join(dir_, 'lanide.db')
...
def test_backups_a_purgar_rotacion():
    nombres = [f'lanide-2026010{d}-000000.db' for d in range(1, 10)]  # 9 backups
    viejos = backups_a_purgar(nombres, max_copias=5)
    assert viejos == [f'lanide-2026010{d}-000000.db' for d in range(1, 5)]
    assert backups_a_purgar(nombres, max_copias=20) == []


def test_backups_a_purgar_ignora_archivos_ajenos():
    nombres = ['lanide-20260101-000000.db', 'notas.txt', 'lanide.db', '.gitkeep']
...
def test_respaldar_db_rota_los_viejos():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        for d in range(1, 8):  # 7 backups pre-existentes, más viejos que el nuevo
            with open(os.path.join(destino, f'lanide-2020010{d}-000000.db'), 'w') as f:
                f.write('x')
        r = respaldar_db(db_path=db, destino_dir=destino, max_copias=5, espejo_dir=None)
        quedan = sorted(n for n in os.listdir(destino) if n.startswith('lanide-'))
        assert len(quedan) == 5
        assert os.path.basename(r['backup']) in quedan
        assert 'lanide-20200101-000000.db' not in quedan
...
def test_espejo_configurable_por_env():
    """LAN_IDE_BACKUP_ESPEJO redirige el espejo (modo app / otras máquinas).
    ...
    previo = os.environ.get('LAN_IDE_BACKUP_ESPEJO')
    try:
        os.environ['LAN_IDE_BACKUP_ESPEJO'] = '/otra/ruta/espejo'
        assert importlib.reload(mant)._BACKUP_ESPEJO == '/otra/ruta/espejo'
        os.environ['LAN_IDE_BACKUP_ESPEJO'] = ''
        assert importlib.reload(mant)._BACKUP_ESPEJO == ''
        os.environ.pop('LAN_IDE_BACKUP_ESPEJO', None)
        assert importlib.reload(mant)._BACKUP_ESPEJO == '/mnt/d/lanide-backups'
    finally:
        if previo is None:
            os.environ.pop('LAN_IDE_BACKUP_ESPEJO', None)
        else:
            os.environ['LAN_IDE_BACKUP_ESPEJO'] = previo
