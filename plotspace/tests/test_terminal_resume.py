...
def test_transcript_existe_detecta_por_glob(tmp_path, monkeypatch):
    ...
    home = tmp_path
    proj = home / '.claude' / 'projects' / '-home-user-lanide'
...
def test_asegurar_session_hook_idempotente(tmp_path):
    ...
    cmd = 'python3 /home/user/lanide/scripts/lanide_claude_hook.py'
...
def test_un_comando_viejo_del_hook_se_REEMPLAZA(tmp_path):
    """REGRESIÓN (2026-07-27): el usuario tenía registrado el comando viejo
        ...
        python3 C:\\Users\\USER\\...\\lanide_claude_hook.py
    ...
    import json as _json
    settings = tmp_path / 'settings.json'
    viejo = 'python3 C:\\Users\\USER\\AppData\\Local\\Lan Ide\\motor\\scripts\\lanide_claude_hook.py'
    settings.write_text(_json.dumps({
        'model': 'x',
        'hooks': {'SessionStart': [{'hooks': [{'type': 'command', 'command': viejo}]}]},
    }))
    nuevo = 'python "C:/proyectos/lanide/scripts/lanide_claude_hook.py"'
...
def test_no_toca_hooks_de_otros(tmp_path):
    ...
    settings.write_text(_json.dumps({
        'hooks': {'SessionStart': [{'hooks': [
            ...
            {'type': 'command', 'command': 'python3 /viejo/scripts/lanide_claude_hook.py'},
        ]}]},
    }))
    nuevo = 'python3 /nuevo/scripts/lanide_claude_hook.py'
