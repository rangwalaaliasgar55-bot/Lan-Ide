...
def _proj(d, archivos: dict):
    for ruta, contenido in archivos.items():
        ...
        with open(path, 'w', encoding='utf-8') as f:
            f.write(contenido)
    os.makedirs(os.path.join(d, '.lan-ide', 'memory'), exist_ok=True)
...
def test_proyecto_vacio_no_detecta_nada():
    with tempfile.TemporaryDirectory() as d:
        ...
        assert not os.path.exists(
            os.path.join(d, '.lan-ide', 'memory', ms.SCAN_BASENAME))
...
def test_sembrar_scan_escribe_memoria_valida():
    with tempfile.TemporaryDirectory() as d:
        ...
        src = open(os.path.join(d, '.lan-ide', 'memory', ms.SCAN_BASENAME)).read()
...
def test_sembrar_scan_no_pisa_existente():
    with tempfile.TemporaryDirectory() as d:
        _proj(d, {'package.json': json.dumps({'scripts': {'dev': 'vite'}})})
        propio = os.path.join(d, '.lan-ide', 'memory', ms.SCAN_BASENAME)
