...
def test_exentos_index_y_lecciones():
    assert gm.es_exento('.lan-ide/memory/INDEX.md')
    assert gm.es_exento('.lan-ide/memory/lecciones-del-enjambre.md')
    assert not gm.es_exento('.lan-ide/memory/una-memoria.md')


def test_es_memoria_detecta_rutas():
    assert gm.es_memoria('.lan-ide/memory/foo.md')
    assert not gm.es_memoria('.lan-ide/memory/INDEX.md')      # exento
    assert not gm.es_memoria('plotspace/core/x.py')
    assert not gm.es_memoria('.lan-ide/MAILBOX.md')
...
def test_memorias_en_rango_caza_lo_commiteado(tmp_path):
    ...
    os.makedirs(os.path.join(d, '.lan-ide', 'memory'))
    ...
    with open(os.path.join(d, '.lan-ide', 'memory', 'colada.md'), 'w') as f:
        f.write('sin frontmatter')
    g('add', '.lan-ide/memory/colada.md'); g('commit', '-qm', 'colada')
