...
Una sesión lanide_<n> es huérfana si no hay una terminal con ese id en la DB.
...
def test_detecta_sesion_sin_terminal_en_db():
    sesiones = ['lanide_1', 'lanide_2', 'lanide_99']
    assert sesiones_huerfanas(sesiones, {1, 2}) == ['lanide_99']


def test_ninguna_huerfana_si_todas_estan_en_db():
    assert sesiones_huerfanas(['lanide_3', 'lanide_4'], {3, 4, 5}) == []


def test_ignora_sesiones_que_no_son_de_card():
    # mobile preview y cualquier sesión con otro formato NO se tocan
    sesiones = ['lanide_mpreview_7', 'otra_cosa', 'lanide_5']
    assert sesiones_huerfanas(sesiones, set()) == ['lanide_5']


def test_db_vacia_todas_las_de_card_son_huerfanas():
    assert sesiones_huerfanas(['lanide_8', 'lanide_9'], set()) == ['lanide_8', 'lanide_9']


def test_tolera_espacios_y_vacios():
    assert sesiones_huerfanas([' lanide_10 ', '', None], {1}) == ['lanide_10']
...
def test_recheck_puntual_salva_terminal_recien_creada():
    ...
    assert huerfanas_a_matar(['lanide_9'], {1}, db_ok=True,
...
def test_recheck_con_error_de_db_no_mata():
    ...
    assert huerfanas_a_matar(['lanide_9'], {1}, db_ok=True, existe_en_db=existe) == []


def test_huerfana_real_se_mata():
    assert huerfanas_a_matar(['lanide_1', 'lanide_9'], {1}, db_ok=True,
                             existe_en_db=lambda tid: False) == ['lanide_9']
...
def test_ejecutar_db_fallida_saltea_tmux_entero():
    ...
    resumen, rec = _con_mantenimiento_parcheado(get_db_roto, ['lanide_1', 'lanide_2'])
...
def test_ejecutar_sin_ids_saltea_tmux_entero():
    resumen, rec = _con_mantenimiento_parcheado(lambda: _FakeConn([]), ['lanide_1'])
    assert resumen['tmux_huerfanas_matadas'] == 0
...
def test_ejecutar_mata_huerfana_real_con_timeout():
    resumen, rec = _con_mantenimiento_parcheado(
        lambda: _FakeConn([1]), ['lanide_1', 'lanide_9'], existe=lambda tid: False)
    ...
    # `=lanide_9`, no `lanide_9`: el `=` es el target EXACTO de tmux. Sin él,
    # tmux resuelve por prefijo — matar "lanide_9" cuando esa sesión ya no
    # existe se lleva puesta "lanide_90", que sí está viva y con un agente
    # adentro. El janitor pasó a usar el motor (`matar_sesion_por_nombre`) y
    # con eso heredó esta guarda, que antes no tenía.
    assert len(kills) == 1 and '=lanide_9' in kills[0][0], kills
...
def test_clientes_attach_muertos_solo_firma_exacta_y_sesion_inexistente():
    procesos = [
        (100, 'tmux attach-session -d -t lanide_838'),   # sesión muerta → matar
        (101, 'tmux attach-session -d -t lanide_929'),   # sesión viva → NO
        (102, 'tmux -C attach-session -t lanide_838'),   # control-mode: otra firma → NO
        (103, 'vim tmux attach-session -d -t lanide_9 x'),  # args con cola → NO (firma anclada)
        (104, 'grep tmux'),                               # cualquier otra cosa → NO
    ]
    assert mantenimiento.clientes_attach_muertos(procesos, ['lanide_929']) == [100]
