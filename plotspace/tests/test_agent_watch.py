...
def test_instruccion_no_suprime():
    # "Cuando termines escribí TASK_DONE" es la instrucción de LanIde, no
...
def test_ocupados_global_vs_scope():
    estados = {
        17: {'fase': 'trabajando'},   # terminal del proyecto LanIde
        48: {'fase': 'idle'},          # terminal de otro proyecto
    }
    assert hay_agentes_ocupados(estados) is True               # global: alguno trabaja
    assert hay_agentes_ocupados(estados, terminal_ids={17}) is True   # LanIde trabaja
    assert hay_agentes_ocupados(estados, terminal_ids={48}) is False  # otro proyecto no cuenta


def test_ocupados_otro_proyecto_no_bloquea_lanide():
    # LanIde idle, otro proyecto trabajando/esperando → scopeado a LanIde = libre.
