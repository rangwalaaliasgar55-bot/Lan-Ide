"""_env_terminal: el entorno con el que nacen TODAS las terminales de LanIde."""
...
def test_prompt_suggestions_apagadas(monkeypatch):
    ...
    toda terminal de LanIde nace con la feature APAGADA. Nombre de la var
    verificado contra el binario 2.1.198 instalado y la doc oficial
    (code.claude.com/docs/en/interactive-mode)."""
    monkeypatch.delenv('LAN_IDE_PROMPT_SUGGESTIONS', raising=False)
    env = _env_terminal()
    assert env['CLAUDE_CODE_ENABLE_PROMPT_SUGGESTION'] == 'false'


def test_prompt_suggestions_escape_hatch(monkeypatch):
    """LAN_IDE_PROMPT_SUGGESTIONS=on restaura el default de Claude Code."""
    monkeypatch.setenv('LAN_IDE_PROMPT_SUGGESTIONS', 'on')
...
def test_claude_fullscreen_por_default(monkeypatch):
    ...
    monkeypatch.delenv('LAN_IDE_CLAUDE_FULLSCREEN', raising=False)
    env = _env_terminal()
    assert env['CLAUDE_CODE_NO_FLICKER'] == '1'


def test_claude_fullscreen_escape_hatch(monkeypatch):
    """LAN_IDE_CLAUDE_FULLSCREEN=off vuelve al default de Claude Code."""
    monkeypatch.setenv('LAN_IDE_CLAUDE_FULLSCREEN', 'off')
...
def test_claude_full_repaint_por_default(monkeypatch):
    ...
    monkeypatch.delenv('LAN_IDE_CLAUDE_FULLSCREEN', raising=False)
    monkeypatch.delenv('LAN_IDE_CLAUDE_FULL_REPAINT', raising=False)
    env = _env_terminal()
    assert env['CLAUDE_CODE_ALT_SCREEN_FULL_REPAINT'] == '1'


def test_claude_full_repaint_escape_hatch(monkeypatch):
    """LAN_IDE_CLAUDE_FULL_REPAINT=off vuelve al render incremental de claude
    (menos bytes) sin apagar el fullscreen."""
    monkeypatch.delenv('LAN_IDE_CLAUDE_FULLSCREEN', raising=False)
    monkeypatch.setenv('LAN_IDE_CLAUDE_FULL_REPAINT', 'off')
