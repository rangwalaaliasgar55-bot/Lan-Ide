...
un hook pre-commit que lee `.lan-ide/LIVE.md`, identifica al agente que
commitea por su sesión tmux (`lanide_<id>`) y BLOQUEA si está por commitear
...
def _con_live(monkeypatch, tmp_path, tid, staged):
    (tmp_path / '.lan-ide').mkdir()
    (tmp_path / '.lan-ide' / 'LIVE.md').write_text(LIVE, encoding='utf-8')
    ...
    monkeypatch.setenv('LAN_IDE_DATA_DIR', str(tmp_path / 'data'))
...
def test_main_permite_sin_tmux(monkeypatch):
    # el usuario commiteando desde su shell (no es lanide_<id>) → permite
    monkeypatch.setattr(g, 'detectar_terminal_id', lambda: None)
    assert g.main() == 0

def test_main_permite_sin_live_md(monkeypatch, tmp_path):
    # falla abierto: sin .lan-ide/LIVE.md no hay nada que proteger
...
# LanIde inyecta al crear la terminal; tmux queda de respaldo para las
# terminales nacidas antes de que existiera.

def test_identidad_prefiere_la_variable_de_entorno(monkeypatch):
    llamadas = []
    monkeypatch.setenv('LAN_IDE_TERMINAL_ID', '441')
...
def test_identidad_cae_a_tmux_sin_variable(monkeypatch):
    class _R:
        returncode = 0
        stdout = 'lanide_77\n'
    monkeypatch.delenv('LAN_IDE_TERMINAL_ID', raising=False)
...
def test_identidad_ignora_una_variable_basura(monkeypatch):
    ...
    monkeypatch.setenv('LAN_IDE_TERMINAL_ID', 'no-soy-un-numero')
    monkeypatch.setattr(g.subprocess, 'run',
