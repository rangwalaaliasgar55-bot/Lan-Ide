...
def test_linea_truncada_sin_parentesis_no_matchea():
    ...
    pane = '● Bash(git add CLAUDE.md .lan-ide/memory/aura-notificacion\n'
    assert extraer_operaciones(pane) == []
...
def test_protocolo_explica_que_msg_no_despierta():
    ...
    extrañan de que el otro no reaccione. Y avisa que `@lanide` no es destinatario
...
def test_asegurar_live_inyecta_protocolo(tmp_path):
    ...
    assert '.lan-ide/LIVE.md' in (tmp_path / '.gitignore').read_text(encoding='utf-8')
...
def test_asegurar_live_gitignore_idempotente(tmp_path):
    ...
    assert gi.count('.lan-ide/LIVE.md') == 1
...
# ─── LanIde pasivo: LIVE nunca escribe en la terminal de un agente ────────────

def test_modulo_no_inyecta_a_terminales():
    # Contrato 2026-06-07: el agente se entera por .lan-ide/LIVE.md (protocolo
    # del CLAUDE.md), LanIde NO le inyecta mensajes. El único que entrega a
