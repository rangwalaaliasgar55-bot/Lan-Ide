...
def test_excluye_puerto_lanide():
    # curl a la API de LanIde / instrucciones que mencionan el dashboard
...
def test_puerto_excluido():
    assert puerto_excluido(3000) is True            # LanIde, siempre
...
def test_proyecto_de_cwd_match_simple():
    proyectos = [{'id': 17, 'ruta': '/home/user/lanide'}]
    assert proyecto_de_cwd('/home/user/lanide/redesigns/chat-ideas', proyectos) == 17
    assert proyecto_de_cwd('/home/user/lanide', proyectos) == 17   # exacto
...
def test_proyecto_de_cwd_sin_match():
    ...
    # prefijo parcial que NO es separador de path no cuenta (lanide ≠ lanide-otro)
    assert proyecto_de_cwd('/home/user/lanide-otro/x', proyectos) is None
...
# ─── Demos servidos por el PROPIO LanIde (:3000/static/<dir>/…) ────────────────

def test_es_demo_lanide_solo_static_no_app():
    from plotspace.core.dev_detect import es_demo_lanide
    # Demos reales: carpetas bajo /static que NO son de la app
    assert es_demo_lanide('http://localhost:3000/static/wb-redesigns/index.html')
    assert es_demo_lanide('http://localhost:3000/static/sidebar-redesign/v2.html')
    assert es_demo_lanide('http://localhost:3000/static/demo-landing/')
    # Superficie de la app: NO son demos
    assert not es_demo_lanide('http://localhost:3000/static/shared/base.css')
    assert not es_demo_lanide('http://localhost:3000/static/sections/preview/preview.js')
    assert not es_demo_lanide('http://localhost:3000/static/shell/workspace.html')
    assert not es_demo_lanide('http://localhost:3000/static/vendor/xterm/xterm.js')
    assert not es_demo_lanide('http://localhost:3000/static/index.html')
    # Fuera de /static o fuera del 3000: no
    assert not es_demo_lanide('http://localhost:3000/api/projects')
    assert not es_demo_lanide('http://localhost:3000/workspace?id=1')
    assert not es_demo_lanide('http://localhost:3000')
    assert not es_demo_lanide('http://localhost:5173/static/demo/')
    assert not es_demo_lanide('basura')


def test_extraer_demos_lanide_del_pane():
    from plotspace.core.dev_detect import extraer_demos_lanide
    texto = (
...
def test_candidatos_excluye_app_lanide_y_oauth():
    texto = (
