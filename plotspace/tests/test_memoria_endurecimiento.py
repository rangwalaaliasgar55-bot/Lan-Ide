...
def test_motivo_matchea_leccion_por_solape():
    leccion = 'Antes de levantar un server chequeá que el puerto esté libre; el 3000 es de LanIde.'
    assert end.motivo_matchea('el puerto 3000 ya estaba ocupado, choqué con LanIde', leccion)
...
def test_contar_reincidencias():
    lecciones = [
        {'slug': 'puerto-libre', 'texto': 'chequeá el puerto libre antes de levantar server, el 3000 es de lanide'},
...
def _mem(d, slug, cuerpo, tags='leccion'):
    mdir = os.path.join(d, '.lan-ide', 'memory')
...
def test_evaluar_persiste_y_marca_candidata():
    with tempfile.TemporaryDirectory() as d:
        _mem(d, 'puerto-libre', 'chequeá el puerto libre antes de levantar el server; el 3000 es de lanide')
