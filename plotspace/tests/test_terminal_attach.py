...
def test_cmd_attach_dueno_desplaza():
    ...
    argv = term._cmd_attach('lanide_999')
...
def test_cmd_attach_observer_no_desplaza_ni_dimensiona():
    ...
    argv = term._cmd_attach('lanide_999', observer=True)
...
def test_observer_no_pisa_terminal_processes():
    ...
    try:
        ...
        assert argv == term._cmd_attach('lanide_999', observer=True)
    finally:
...
def test_filtrar_detach_guard_byte_identico():
    ...
    sucio = 'antes\r[detached (from session lanide_510)]\r\ndespués'
