...
def test_matar_sesion_target_exacto_y_verificacion(monkeypatch):
    """kill-session usa '=lanide_N' (sin '=', tmux resuelve por PREFIJO cuando
    no hay match exacto: matar lanide_1 muerta podía llevarse lanide_12 viva) y
    ...
    assert '=lanide_5' in kill, kill
    chk = next(c for c in llamadas if 'has-session' in c)
    assert '=lanide_5' in chk, chk
...
def test_matar_sesion_ya_muerta_es_idempotente(monkeypatch):
    async def fake_tmux(*args, **kw):
        if 'kill-session' in args:
            return _Res(1, "can't find session: lanide_5")
...
def test_reconciliar_mata_zombies_activa_cero(monkeypatch):
    ...
    assert len(kills) == 1 and '=lanide_3' in kills[0], kills
    assert 'lanide_3' not in vivas and 'lanide_7' in vivas
    # la sesión ajena no-lanide ni se toca ni pierde membresía
    assert 'sesion_personal' in vivas
    # las lanide_* vivas legítimas siguen recibiendo window-size/status
    ...
    opts = [' '.join(c) for c in corridas if 'set-option' in c and 'lanide_7' in ' '.join(c)]
