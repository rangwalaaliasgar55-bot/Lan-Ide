...
Claude reporta por su hook PostToolUse; opencode por un PLUGIN JS que LanIde deja
...
def test_instala_el_plugin_de_opencode(tmp_path):
    ...
    p = tmp_path / 'plugin' / 'lanide-swarm.js'
    assert p.exists()
    txt = p.read_text(encoding='utf-8')
    assert '/api/swarm/op' in txt and 'LAN_IDE_TERMINAL_ID' in txt
...
def test_reescribe_si_cambio(tmp_path):
    ...
    dest.mkdir()
    (dest / 'lanide-swarm.js').write_text('plugin viejo', encoding='utf-8')
...
def test_no_rompe_sin_fuente(tmp_path):
    """Sin scripts/opencode_lanide_plugin.js en la raíz dada: False, no excepción."""
...
def test_instala_el_hook_de_qwen(tmp_path):
    ...
    assert 'lanide_ops_hook' in post['hooks'][0]['command']  # reusa el hook de Claude
...
def test_instala_el_hook_de_antigravity(tmp_path):
    ...
    nuestro = data['lanide-provenance']
    ...
    assert 'lanide_ops_hook' in grupo['hooks'][0]['command']
...
def test_antigravity_no_pisa_hooks_ajenos(tmp_path):
    ...
    assert 'lanide-provenance' in data       # el nuestro, al lado
...
#                con `.lan-ide/jv estado`, que funciona en cualquier terminal.
...
def test_el_hook_de_antigravity_sigue_siendo_solo_pretooluse(tmp_path):
    ...
    hooks_json = tmp_path / 'hooks.json'
    cli_adapters.asegurar_antigravity_hook(RAIZ, str(hooks_json))
    mio = json.loads(hooks_json.read_text(encoding='utf-8'))['lanide-provenance']
