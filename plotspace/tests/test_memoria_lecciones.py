...
`.lan-ide/memory/lecciones-del-enjambre.md`, y ese contenido se inyecta entre
...
def test_destilar_sin_key_degrada():
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        assert not os.path.exists(os.path.join(d, '.lan-ide', 'memory', lec.LECCIONES_BASENAME))
...
def test_destilar_escribe_leccion_e_inyecta():
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        assert r['ok'] is True
        archivo = os.path.join(d, '.lan-ide', 'memory', lec.LECCIONES_BASENAME)
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        try:
            ...
            if key_prev is None:
                ...
                os.environ['ANTHROPIC_API_KEY'] = key_prev
        assert r['ok'] is True
        src = open(os.path.join(d, '.lan-ide', 'memory', lec.LECCIONES_BASENAME)).read()
