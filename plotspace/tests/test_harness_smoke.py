...
def main():
    # fresh_db apunta a un tempfile, NO a lanide.db
    db_path = fresh_db()
    assert database.DB_PATH == db_path, "DB_PATH no quedó repuntado al tempfile"
    assert "lanide_test_" in os.path.basename(db_path), "el tempfile no tiene el prefijo esperado"
    assert os.path.basename(db_path) != "lanide.db", "fresh_db apuntó a la DB real"
