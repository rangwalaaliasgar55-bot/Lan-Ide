...
# ── _titulos_vivos_tmux: una pasada, solo sesiones lanide_{id} numéricas ─────

def test_titulos_vivos_parsea_una_sola_pasada():
    salida = (
        'lanide_471\t✳ Remove terminal output\n'
        'lanide_472\t⠐ Fix layout bug when resizing menus in terminals\n'
        'lanide_473\tHOST\n'                       # CLI sin título → genérico
        'lanide_mpreview_18\tnpx expo start\n'     # no es terminal de card → fuera
        'otra_sesion\tlo que sea\n'                # ajena a lanide → fuera
    )
