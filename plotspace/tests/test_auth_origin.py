...
def test_host_dominio_externo_rechazado():
    ...
    assert not host_permitido('lanide.attacker.com:3000')
...
def test_host_extra_allowlist():
    # Escape hatch para un dominio local configurado (LAN_IDE_ALLOWED_HOSTS)
    assert host_permitido('mi-lanide.local:3000', extra=('mi-lanide.local',))
    assert not host_permitido('mi-lanide.local:3000', extra=('otro.local',))
...
def test_origin_extra_allowlist():
    assert origen_permitido('http://mi-lanide.local:3000', extra=('mi-lanide.local',))
