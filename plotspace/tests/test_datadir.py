# LAN_IDE_DATA_DIR — el directorio de datos sale del repo cuando la app lo pide.
#
# Modo app (shell de escritorio): el shell setea LAN_IDE_DATA_DIR y TODO el
...
def _env_datadir(valor):
    """Setea/borra LAN_IDE_DATA_DIR, recarga datadir, y SIEMPRE restaura al
    salir (env + reload) para no contaminar al resto de la suite."""
    previo = os.environ.get('LAN_IDE_DATA_DIR')
    try:
        if valor is None:
            os.environ.pop('LAN_IDE_DATA_DIR', None)
        else:
            os.environ['LAN_IDE_DATA_DIR'] = valor
        yield importlib.reload(datadir)
    finally:
        if previo is None:
            os.environ.pop('LAN_IDE_DATA_DIR', None)
        else:
            os.environ['LAN_IDE_DATA_DIR'] = previo
...
def test_ruta_data_une_segmentos(tmp_path):
    with _env_datadir(str(tmp_path)) as dd:
        assert dd.ruta_data('lanide.db') == str(tmp_path / 'lanide.db')
...
def test_consumidores_siguen_el_data_dir(tmp_path):
    ...
    try:
        with _env_datadir(str(tmp_path)):
            for m in mods:
                importlib.reload(m)
            assert database.DB_PATH == str(tmp_path / 'lanide.db')
            assert logs._LOG_PATH == str(tmp_path / 'lanide.log')
...
def test_upload_dir_usa_el_tempdir_del_sistema(tmp_path):
    ...
    try:
        ...
        assert t2.UPLOAD_DIR == str(tmp_path / 'lanide_uploads')
    finally:
        _tf.tempdir = previo
