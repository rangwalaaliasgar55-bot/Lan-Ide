...
def test_foto_captura_wip_y_no_toca_el_indice_real():
    with tempfile.TemporaryDirectory() as d:
        ...
        ref = snapshot_wip(d)
        assert ref and ref.startswith('refs/lanide/wip/')
...
def test_poda_acota_las_fotos():
    with tempfile.TemporaryDirectory() as d:
        ...
        for i in range(4):
            ...
            assert ref
            _git(d, 'update-ref', f'refs/lanide/wip/fake-{i:02d}',
                 ref.split('/')[-1] and _git(d, 'rev-parse', ref)[1])
        rc, out = _git(d, 'for-each-ref', '--format=%(refname)', 'refs/lanide/wip')
...
def test_un_lock_huerfano_no_deja_el_paracaidas_muerto_para_siempre():
    """El bug real: una corrida interrumpida dejó `lanide-wip-index.lock` y
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        with open(os.path.join(d, 'wip.txt'), 'w') as f:
            f.write('x\n')
        lock = os.path.join(d, '.git', 'lanide-wip-index.lock')
        open(lock, 'w').close()
