...
# ─── Atribución: trailer LanIde-Agent (recuperar de quién es un commit) ───────
...
def test_trailer_atribucion_formato():
    assert cp.trailer_atribucion('Backend', 100) == 'LanIde-Agent: Backend (tid 100)'
...
def test_trailer_atribucion_sin_nombre_usa_el_tid():
    ...
    assert cp.trailer_atribucion(None, 42) == 'LanIde-Agent: terminal 42 (tid 42)'
