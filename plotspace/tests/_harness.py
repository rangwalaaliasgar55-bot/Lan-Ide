...
Aísla cada test en una base de datos SQLite temporal (nunca toca lanide.db) y
...
def fresh_db() -> str:
    ...
    No ensucia lanide.db. Devuelve la ruta del archivo temporal por si el
    ...
    """
    fd, path = tempfile.mkstemp(suffix=".db", prefix="lanide_test_")
    os.close(fd)
