...
Capa Captura del sistema de memoria (parte 2):
  - lanide.log conserva DOS generaciones al rotar (.1 → .2) en vez de pisar .1
  - el canary de /api/system/restart deja el traceback en el audit trail
    (antes viajaba solo en el body del 409 y se perdía)
  - guard_propiedad registra cada commit bloqueado en data/lanide.log
...
def test_guard_registra_bloqueo():
    ...
    with tempfile.TemporaryDirectory() as raiz:
        ...
        # Aislar del entorno real: sin LAN_IDE_DATA_DIR cae a <raiz>/data
        env_prev = os.environ.pop('LAN_IDE_DATA_DIR', None)
        try:
            guard.registrar_bloqueo(raiz, 7, viol)
        finally:
            if env_prev is not None:
                os.environ['LAN_IDE_DATA_DIR'] = env_prev

        log = os.path.join(raiz, 'data', 'lanide.log')
        assert os.path.exists(log), "el bloqueo debe dejar rastro en data/lanide.log"
