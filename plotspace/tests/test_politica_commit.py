...
def test_salida_de_build_es_artefacto():
    assert pol.clasificar('desktop/dist/lanide.exe') == 'artefacto'
    assert pol.clasificar('desktop/dist/bundle.wsl') == 'artefacto'


def test_capturas_de_qa_son_artefacto():
    assert pol.clasificar('.lan-ide/qa-shots/radio-8.png') == 'artefacto'


def test_estado_de_runtime_es_artefacto():
    for p in ('server.pid', 'data/lanide.db', '.workspace/logs/terminal_1.log',
...
def test_las_memorias_del_enjambre_son_trabajo_real():
    ...
    assert pol.clasificar('.lan-ide/memory/cli-jv-enjambre.md') == 'real'
...
def test_filtrar_deja_solo_lo_real():
    entrada = ['plotspace/x.py', 'desktop/dist/a.exe', 'frontend/preview-y/i.html',
               '.lan-ide/memory/z.md']
    assert pol.solo_real(entrada) == ['plotspace/x.py', '.lan-ide/memory/z.md']
