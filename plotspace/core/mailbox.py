# LAN_IDE — Mailbox entre agentes.
# Buzón compartido en .lan-ide/MAILBOX.md del proyecto: los agentes se dejan
...
MAILBOX_NOMBRE = os.path.join('.lan-ide', 'MAILBOX.md')
...
PROTOCOLO_MARKER_START = '<!-- LAN_IDE_MAILBOX_START -->'
PROTOCOLO_MARKER_END   = '<!-- LAN_IDE_MAILBOX_END -->'
...
ARCHIVO_NOMBRE = os.path.join('.lan-ide', 'MAILBOX-archivo.md')
    ...
    if not os.path.exists(archivo):
        ...
        with open(archivo, 'w', encoding='utf-8') as f:
            f.write('# Mailbox del workspace\n'
                    '<!-- - @De -> @Para: mensaje  ·  canal por archivo: leé .lan-ide/MAILBOX.md -->\n')
    ...
    try:
        ...
        if os.path.exists(gi):
            with open(gi, encoding='utf-8') as f:
                actual = f.read()
        faltan = [e for e in ('.lan-ide/mailbox-pendientes.json',)
                  if e not in actual]
        if faltan:
...
def armar_digest(msgs: list) -> str:
    ...
    if len(msgs) > 6:
        lineas.append(f"  (+{len(msgs) - 6} más en .lan-ide/MAILBOX.md)")
    lineas.append("Respondé por .lan-ide/MAILBOX.md si hace falta y seguí con lo tuyo.")
    return '\n'.join(lineas)
...
async def _entregar(project_id: int, de: str, para: str, msg: str):
    ...
    # pedido del usuario: el chat es SU canal con LanIde y el tráfico de
...
PENDIENTES_NOMBRE = os.path.join('.lan-ide', 'mailbox-pendientes.json')


def _escribir_pendientes_json(project_ruta: str, pendientes: list):
    """Refleja los pendientes en .lan-ide/mailbox-pendientes.json (por
