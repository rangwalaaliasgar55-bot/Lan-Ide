...
shell (`.lan-ide/signals/terminal_<id>.json` con {estado, motivo}). El contenido
...
def parsear(texto):
    ...
    `memorias_usadas` (slugs de .lan-ide/memory/ que el agente leyó) mide si la
...
def ruta_sentinel(project_ruta, terminal_id):
    ...
    return os.path.join(project_ruta, '.lan-ide', 'signals', f'terminal_{terminal_id}.json')
...
def instruccion_cierre(terminal_id):
    ...
    return (
        ...
        f"  mkdir -p .lan-ide/signals && printf '%s' '{{\"estado\":\"done\",\"memorias_usadas\":[]}}' > .lan-ide/signals/terminal_{terminal_id}.json\n"
        "En \"memorias_usadas\" listá los slugs de .lan-ide/memory/ que leíste y te "
