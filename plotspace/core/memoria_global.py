...
Cada proyecto nuevo del workspace arranca con `.lan-ide/memory/` vacía: el
enjambre re-tropieza con lo mismo (WSL no reenvía puertos, el 3000 es de
LanIde, el índice git es compartido) hasta que lo aprende de nuevo. Este
...
# Lecciones de entorno universales (mismas slugs que las canónicas de LanIde,
# así en el propio repo NO se duplican — sembrar salta lo que ya existe).
SEMILLA = [
    {
        'slug': 'regla-de-puertos',
        'titulo': 'Regla de puertos: el 3000 es de LanIde — chequeá antes de servir',
        'categoria': 'entorno',
        'tags': ['puertos', 'dev-server', 'entorno'],
        'cuerpo': (
            "El puerto **3000 está PROHIBIDO**: ahí corre Lan Ide. Antes de "
...
def _mem_dir(project_path: str) -> str:
    return os.path.join(project_path, '.lan-ide', 'memory')
