# LAN_IDE — Protocolo de puertos para agentes.
...
# de Lan Ide y NINGÚN agente puede levantar nada ahí; antes de
...
# Puerto de Lan Ide: jamás se mata (rompería el dashboard que orquesta).
PUERTO_LAN_IDE = 3000

PROTOCOLO_MARKER_START = '<!-- LAN_IDE_PUERTOS_START -->'
PROTOCOLO_MARKER_END   = '<!-- LAN_IDE_PUERTOS_END -->'
...
# usuario 2026-06-18). Guard duro: NUNCA el 3000 (LanIde) ni el propio PID.
...
def matar_puerto(port) -> dict:
    ...
    NUNCA mata el 3000 (LanIde) ni el propio proceso. Devuelve
    ...
    if port == PUERTO_LAN_IDE:
        return {'ok': False, 'pids': [], 'motivo': 'es el puerto de LanIde (3000), no se mata'}
    ...
    for pid in pids:
        if pid == yo:
            continue   # jamás el propio uvicorn de LanIde
        try:
