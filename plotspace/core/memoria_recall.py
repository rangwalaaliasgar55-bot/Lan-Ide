...
_RUIDO = {'lanide', 'workspace', 'proyecto', 'usuario', 'gotcha', 'regla',
...
def bloque_relevantes(project_path: str, archivos: list, tarea: str, k: int = 5,
    ...
    for m in rel:
        ...
        lineas.append(f"  • .lan-ide/memory/{m['slug']}.md — {m['titulo']}{marca}")
    ...
    lineas.append("(índice completo: .lan-ide/memory/INDEX.md)")
    return '\n'.join(lineas)
