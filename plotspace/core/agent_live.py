...
archivo que las CLIs imprimen (`● Update(path)` etc.). Con eso mantiene:
  ...
  - `.lan-ide/LIVE.md` del proyecto (regenerado completo, gitignored)

LanIde es 100% PASIVO acá (contrato 2026-06-07): NUNCA escribe en la
terminal de un agente. El agente sabe por el protocolo de su CLAUDE.md
que tiene `.lan-ide/LIVE.md` para mirar quién toca qué; los conflictos
...
capturá su pane real (`tmux capture-pane -t lanide_<id> -p -S -120`) y
sumá su regex + test.
"""
...
LIVE_NOMBRE = os.path.join('.lan-ide', 'LIVE.md')

PROTOCOLO_MARKER_START = '<!-- LAN_IDE_LIVE_START -->'
PROTOCOLO_MARKER_END   = '<!-- LAN_IDE_LIVE_END -->'
...
def generar_live_md(snapshot: dict, ahora_str: str) -> str:
    """Markdown de .lan-ide/LIVE.md a partir de un snapshot. Solo writes
...
def asegurar_live(project_path: str):
    ...
    patrón que asegurar_mailbox) + agrega .lan-ide/LIVE.md al .gitignore
    ...
    try:
        ...
        if os.path.exists(gitignore):
            with open(gitignore, encoding='utf-8') as f:
                gi = f.read()
        faltan = [e for e in ('.lan-ide/LIVE.md', '.lan-ide/jv') if e not in gi]
        if faltan:
    ...
    # CLI del enjambre disponible en el proyecto (`.lan-ide/jv`): el agente
    ...
    try:
        ...
        if swarm_cli.asegurar_jv(project_path, raiz):
            print(f'[agent-live] CLI jv instalado en {project_path}/.lan-ide/jv')
    except Exception as e:
