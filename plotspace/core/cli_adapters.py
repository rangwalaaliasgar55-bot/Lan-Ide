...
sus hunks arrastrados por `commit_propio`).
  ...
  opencode → un PLUGIN JS (hook `tool.execute.after`) en `~/.config/opencode/plugin/`.
             ...
             tmux. Fuente: `scripts/opencode_lanide_plugin.js`.
...
Reglas del instalador (las mismas que hooks_cli, aprendidas de romper cosas):
  ...
  · No toca plugins ajenos del usuario: escribe un archivo con NOMBRE propio
    (`lanide-swarm.js`), nunca un dir compartido — el glob de opencode
...
NOMBRE_PLUGIN_SRC = 'opencode_lanide_plugin.js'      # fuente en scripts/
NOMBRE_PLUGIN_DEST = 'lanide-swarm.js'               # nombre instalado (propio)
...
def asegurar_opencode_plugin(raiz_repo: str, destino_dir: str = None) -> bool:
    """Copia `scripts/opencode_lanide_plugin.js` a `<destino_dir>/lanide-swarm.js`
...
# ─── qwen: hook estilo Claude (reusa lanide_ops_hook.py) ──────────────────────
...
# stdin. Así que se reusa TAL CUAL el mismo hook (scripts/lanide_ops_hook.py) — solo
...
def asegurar_qwen_hook(raiz_repo: str, settings_path: str = None) -> bool:
    ...
    mismo lanide_ops_hook.py que Claude, con los tool names de qwen). Idempotente,
...
#      que lanide_ops_hook contesta `{"decision":"allow"}` siempre.
_AGY_NOMBRE_HOOK = 'lanide-provenance'
