# LAN_IDE — candado de Host / Origin (anti DNS-rebinding y CSWSH).
...
def host_permitido(host_header: str | None, extra=()) -> bool:
    ...
    —incluida la IP de LAN— y cualquier host extra (env LAN_IDE_ALLOWED_HOSTS).
...
def hosts_extra() -> tuple:
    ...
    en /etc/hosts), separada por comas en env LAN_IDE_ALLOWED_HOSTS."""
    crudo = os.environ.get('LAN_IDE_ALLOWED_HOSTS', '').strip()
...
def imprimir_banner():
    ...
    print('[lanide] listo — abrí http://127.0.0.1:3000')
