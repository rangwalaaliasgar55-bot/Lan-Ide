...
def parsear_panes(salida) -> dict:
    ...
    CLI). Ignora todo lo que no sea una sesión `lanide_<n>`."""
    out = {}
    for linea in (salida or '').splitlines():
        ...
        sesion = sesion.strip()
        if not sesion.startswith('lanide_'):
            continue
        try:
            tid = int(sesion[len('lanide_'):])
        except ValueError:
            continue
...
def estados_ahora(terminales, fases) -> dict:
    ...
    (b) tmux reportó al menos UNA sesión de LanIde. Sin (b), lo más probable no
