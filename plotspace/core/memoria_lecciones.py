...
`.lan-ide/memory/lecciones-del-enjambre.md`: ≤20 reglas cortas de prevención
...
MARKER_START = '<!-- LAN_IDE_LECCIONES_START -->'
MARKER_END   = '<!-- LAN_IDE_LECCIONES_END -->'
...
def _ruta_lecciones(project_path: str) -> str:
    return os.path.join(project_path, '.lan-ide', 'memory', LECCIONES_BASENAME)
...
def lecciones_de_memorias(project_path: str, k: int = 12) -> list:
    ...
    d = os.path.join(project_path, '.lan-ide', 'memory')
    try:
    ...
    for nombre in nombres:
        ...
        lecciones.append((fecha, f"- {linea[:200]} — .lan-ide/memory/{nombre}"))
