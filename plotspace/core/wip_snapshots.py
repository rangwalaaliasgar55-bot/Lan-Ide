...
refs internas (`refs/lanide/wip/<ts>`) usando un ÍNDICE TEMPORAL — jamás
toca el índice real (que es de los agentes) ni crea commits en ninguna rama.

Recuperar un archivo barrido:
    git for-each-ref refs/lanide/wip            # listar fotos
...
def _limpiar_lock(idx: str):
    ...
    dejó `lanide-wip-index.lock`, y desde entonces TODOS los intentos morían en
    ...
    solo uso: nada más en el repo toca `lanide-wip-index`. Ojo: jamás tocar
...
def snapshot_wip(repo: str, keep: int = KEEP):
    """Foto del árbol sucio a refs/lanide/wip/<ts>. None si el árbol está limpio
    ...
    try:
        ...
        if not sucio.strip():
            ...
            return None
        idx = os.path.join(repo, '.git', 'lanide-wip-index')
        ...
        try:
            ...
            rc, commit = _git(repo, 'commit-tree', tree, '-p', head,
                              '-m', 'wip snapshot (paracaídas de LanIde)')
            if rc != 0:
                return _fallar(f'commit-tree falló en {repo}')
            ts = datetime.now().strftime('%Y%m%d-%H%M%S')
            ref = f'refs/lanide/wip/{ts}'
...
def _podar(repo: str, keep: int):
    ...
    rc, out = _git(repo, 'for-each-ref', '--format=%(refname)', 'refs/lanide/wip')
    if rc != 0:
        return
