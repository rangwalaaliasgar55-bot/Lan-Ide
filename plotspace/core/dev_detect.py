...
# sirviéndolo por LanIde (WSL no reenvía puertos nuevos a Windows — patrón
# "demos vía /static"). No son procesos: "viven" mientras sus archivos existan
# y "cerrarlos" = ocultarlos del menú (JAMÁS se toca el 3000, es LanIde mismo).
...
def es_demo_lanide(url: str) -> bool:
    """¿La URL es un demo servido por el propio LanIde? = :3000/static/<dir>/…
...
def extraer_demos_lanide(texto: str) -> list:
    """URLs de demos del propio LanIde presentes en el texto de un pane, más
...
def extraer_urls_locales(texto: str, es_expo: bool = False) -> list:
    ...
    Excluye el puerto de LanIde siempre, y Metro (:8081) en proyectos Expo
...
def extraer_candidatos_pane(texto: str, es_expo: bool = False) -> list:
    """Dev servers Y demos del propio LanIde presentes en el texto de un pane,
    ...
    líneas de login) y extraer_demos_lanide (solo /static/<dir> fuera de la
...
# detached/background (huérfanos bajo /init, subprocesos del propio LanIde, el
...
def servers_detectados(project_id: int) -> list:
    ...
    (página servida por el propio LanIde). Lo consume el menú de localhost."""
    _asegurar_cargado()
    return [
...
async def _ciclo():
    ...
    async def _sigue_vivo(pid, url):
        # Un demo del propio LanIde vive mientras existan sus archivos (el
    ...
    for pid in list(_descartadas.keys()):
        ...
        vivas = {u for u in _descartadas[pid]
                 if (demo_vivo(u) if es_demo_lanide(u) else puerto_de(u) in listen_ports)}
        if vivas:
    ...
    for row in rows:
        ...
        if not texto:
            continue
        # Demos del propio LanIde (:3000/static/<dir>/…): un agente dejó un
        ...
        for url in reversed(extraer_demos_lanide(texto)):
    ...
    #    propio LanIde, el Bash de un agente). Los cazamos por el PUERTO real en
    ...
    candidatos = {p for p in listen_ports if p != PUERTO_LAN_IDE} - _puertos_ya_conocidos()
    if not candidatos:
        return
