...
QUÉ ESTABA MAL (medido sobre este proyecto)
  · `.lan-ide/MAILBOX.md` son 46 KB / 11,6K tokens y el protocolo le pide a cada
...
El canal sigue siendo `.lan-ide/MAILBOX.md` (append-only, una línea por
...
async def enviar(terminal_id, para, msg, espera=False):
    ...
    archivo = os.path.join(yo['ruta'] or '', '.lan-ide', 'MAILBOX.md')
...
JV_NOMBRE = os.path.join('.lan-ide', 'jv')


def asegurar_jv(project_path: str, raiz_lanide: str) -> bool:
    """Deja un ejecutable `.lan-ide/jv` en el proyecto que reenvía al CLI real.

    Así el agente corre `.lan-ide/jv inbox` en CUALQUIER proyecto sin tocarle el
    ...
    real = os.path.join(raiz_lanide, 'scripts', 'jv.py')
    contenido = f'#!/bin/sh\n# Generado por LanIde — CLI del enjambre.\nexec python3 "{real}" "$@"\n'
    ...
    contenido_cmd = (
        '@echo off\r\n'
        f'rem Generado por LanIde — CLI del enjambre.\r\n'
        f'python "{real}" %*\r\n'
    )
    try:
