"""
LAN_IDE — Mantenimiento del estado local.
...
# Solo sesiones de card (lanide_<id>). El mobile preview u otras sesiones con
# otro nombre NO matchean y por lo tanto nunca se las considera huérfanas.
_SESION_RE = re.compile(r'^lanide_(\d+)$')
_STALE_HORAS = 12
...
def sesiones_huerfanas(sesiones_tmux: list, ids_db) -> list:
    """Nombres de sesión `lanide_<n>` que NO corresponden a ninguna terminal
...
def huerfanas_a_matar(sesiones_tmux: list, ids_db, db_ok: bool, existe_en_db=None) -> list:
    ...
    terminales está incompleta y TODA sesión lanide_* parecería huérfana → no se
...
# El borrado accidental del repo se llevó data/lanide.db y fue lo ÚNICO
...
_BACKUP_DIR = os.path.join(os.path.expanduser('~'), '.lan-ide-backups')
...
# tiene /mnt/d).
_BACKUP_ESPEJO = os.environ.get('LAN_IDE_BACKUP_ESPEJO', '/mnt/d/lanide-backups')
_BACKUP_MAX = 48          # ~1 día de historia a 1 backup/30 min (la DB pesa KB)
_BACKUP_RE = re.compile(r'^lanide-\d{8}-\d{6}\.db$')


def backups_a_purgar(nombres: list, max_copias: int) -> list:
    """Backups que SOBRAN (los más viejos) para dejar a lo sumo max_copias.
    Solo considera los nuestros (lanide-YYYYMMDD-HHMMSS.db; el timestamp del
...
def respaldar_db(db_path: str = None, destino_dir: str = None,
    ...
    """Snapshot de la DB a destino_dir (default ~/.lan-ide-backups) + espejo
    ...
    if not os.path.isfile(db_path):
        return resumen
    nombre = datetime.now().strftime('lanide-%Y%m%d-%H%M%S.db')
...
# 55 clientes `tmux attach-session -d -t lanide_N` del motor classic colgados
...
#    kill directo: son siempre basura de LanIde, cero riesgo.
...
_ATTACH_RE = re.compile(r'tmux attach-session -d -t (lanide_\d+)$')
...
def clientes_attach_muertos(procesos: list, sesiones_vivas) -> list:
    """PIDs de clientes `tmux attach-session -d -t lanide_N` cuya sesión ya NO
...
def ejecutar_mantenimiento(keep_eventos: int = 5000) -> dict:
    ...
    # incompleta y TODAS las lanide_* parecerían huérfanas → se saltea entero.
    if not db_ok or not ids_db:
...
def mantener_memorias() -> dict:
    ...
    for ruta in rutas:
        if not os.path.isdir(os.path.join(ruta or '', '.lan-ide', 'memory')):
            continue
...
async def poller_purga_logs():
    ...
    while True:
        ...
        try:
            ...
            # proyecto a refs/lanide/wip (índice temporal — jamás el real).
