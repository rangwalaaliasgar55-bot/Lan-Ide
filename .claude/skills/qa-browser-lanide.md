---
name: qa-browser-lanide
description: How to verify Lan Ide in a real browser (Playwright) and run all its tests. Use it before closing any frontend change.
---

# Browser QA for Lan Ide
...
- Top bar: `#jw-bar` · dock toggle `#jw-dock-toggle` · gear `#jw-gear`
  - Center: breadcrumb `<nav class="gh-crumb-center">` "Lan Ide › project"
  - **`#btn-new-terminal` does NOT exist** (removed in the redesign)
- Dock: `#jw-dock` + `.jw-tab[data-tab=preview|lanide|editor|tasks|review|mobile]`
...
- Themes: `html[data-theme=<name>]` (default violet = no attribute); change via `LanIdeThemes.aplicar('<name>')`
- JS API in console: `LanIdeDock.open/setTab/notify/isOpen/activeTab`, `WebPreview.setUrl/detectar`, `LanIdeSettings.open`, `LanIdeThemes.aplicar/actual`
