#!/usr/bin/env bash
# LAN_IDE — Reinicio del server SIN robarle el proceso a la terminal del usuario.
#
# ⚠️ HERRAMIENTA DEL USUARIO (o de LanIde mismo). LOS AGENTES NO REINICIAN EL
...
# `setsid` y no solo `nohup`: cuando quien nos llama es LanIde.exe (o el .bat),
