...
el token de LanIde ni credenciales de ningún proveedor: son de la persona y
...
2. Los VALORES REALES de los secretos locales (data/lanide_token.txt,
...
def valores_locales(raiz):
    ...
    vals = []
    try:
        tok = open(os.path.join(raiz, 'data', 'lanide_token.txt'),
                   encoding='utf-8').read().strip()
        if len(tok) > 12:
            vals.append(('token-lanide', tok))
    except OSError:
        pass
    try:
