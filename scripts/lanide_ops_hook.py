...
# POR QUÉ
# -------
# LanIde deducía qué archivo tocaba cada agente parseando el pane tmux en busca
...
#   PreToolUse   → POST /api/swarm/check : pregunta ANTES de escribir; si LanIde
#                  contesta que no, se DENIEGA la herramienta con el motivo.
#
# DOCTRINA: best-effort absoluto. Sin LAN_IDE_TERMINAL_ID (claude fuera de
# LanIde), sin server, con timeout o con cualquier excepción → NO-OP que deja
...
# descarta, y el intento se come el timeout entero. Sin esto, con LanIde caído
...
def _dir_datos():
    return os.environ.get("LAN_IDE_DATA_DIR") or os.path.join(
        os.path.expanduser("~"), "lanide", "data")
...
def _anfitriones():
    ...
    Se puede fijar a mano con LAN_IDE_HOST, que gana sobre todo lo demás.
    """
    fijo = (os.environ.get("LAN_IDE_HOST") or "").strip()
    if fijo:
        return [fijo]
    hosts = ["127.0.0.1"]
    try:
...
def _get(ruta, timeout):
    """GET a LanIde con el mismo corta-corriente que _post. None si está abierto."""
    data_dir = _dir_datos()
    if _corta_corriente_abierto(data_dir):
        return None
    try:
        out = _http_get(os.environ.get("LAN_IDE_PORT", "3000"), ruta, timeout)
    except Exception:
...
def _post(ruta, cuerpo, timeout):
    """POST JSON a LanIde. Devuelve el dict de respuesta, o None si el corta-
    ...
    if _corta_corriente_abierto(data_dir):
        return None
    try:
        out = _http_crudo(os.environ.get("LAN_IDE_PORT", "3000"), ruta, cuerpo, timeout)
    except Exception:
        _marcar_caido(data_dir, True)
        raise
    _marcar_caido(data_dir, False)
...
def main():
    tid = os.environ.get("LAN_IDE_TERMINAL_ID")
    if not tid:
        return                      # claude fuera de LanIde → no-op
    try:
