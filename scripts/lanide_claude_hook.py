#!/usr/bin/env python3
# SessionStart hook de Claude Code para LanIde.
#
# claude rota su transcript (nuevo <uuid>.jsonl) cada vez que la conversación se
# compacta o se continúa. Si LanIde guardó el uuid INICIAL, al reanudar con
# --resume trae contexto viejo/parcial. Este hook, que claude dispara en CADA
# arranque (startup/resume/clear), le avisa a LanIde el session-id VIVO para que
...
# El hook solo actúa si corre dentro de una terminal de LanIde (LAN_IDE_TERMINAL_ID
# en el env, que LanIde setea en la sesión tmux). Best-effort ABSOLUTO: cualquier
...
def main() -> None:
    ...
    if not tid:
        return  # claude normal fuera de LanIde → no-op
    try:
