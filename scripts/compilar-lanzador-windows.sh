#!/usr/bin/env bash
# Compila LanIde.exe — el SHELL de escritorio fino (scripts/lanide-shell.cs) —
...
#   $HOME/lan-ide   (override: LAN_IDE_WSL_DIR)
#   distro default de WSL    (override: LAN_IDE_WSL_DISTRO)
#
...
TMP="/mnt/c/Users/${WINUSER}/AppData/Local/Temp/lanide-shell-build"

mkdir -p "$TMP"
cp lanide-shell.cs lanide.ico "$TMP/"
cp lanide-shell-ui.html "$TMP/ui.html"      # el splash de siempre (recurso de texto)
...
( cd "$TMP" && "$CSC" /nologo /target:winexe /win32icon:lanide.ico \
    /r:System.Windows.Forms.dll /r:System.Drawing.dll \
    /r:System.Web.Extensions.dll \
    /r:Microsoft.Web.WebView2.Core.dll /r:Microsoft.Web.WebView2.WinForms.dll \
    /resource:lanide.ico /resource:ui.html \
    /out:LanIde.exe lanide-shell.cs )
...
# el usuario abre.
APP="/mnt/c/Users/${WINUSER}/AppData/Local/LanIde"
mkdir -p "$APP"
...
# toma el nuevo. Sin esto, compilar con LanIde abierto abortaba a mitad de la
...
for f in "$TMP/LanIde.exe" "$TMP/lanide.ico" \
...
APP_WIN="C:\\Users\\${WINUSER}\\AppData\\Local\\LanIde"
DESTINO_WIN=$(wslpath -w "$DESTINO")
PS1="$APP/crear-acceso.ps1"
cat > "$PS1" <<PSEOF
\$s = (New-Object -ComObject WScript.Shell).CreateShortcut('${DESTINO_WIN}\\LanIde.lnk')
\$s.TargetPath = '${APP_WIN}\\LanIde.exe'
\$s.IconLocation = '${APP_WIN}\\lanide.ico,0'
\$s.WorkingDirectory = '${APP_WIN}'
\$s.Description = 'Lan Ide'
\$s.Save()
PSEOF
...
echo "listo: ${APP}/LanIde.exe"
echo "       acceso directo → ${DESTINO}/LanIde.lnk"
