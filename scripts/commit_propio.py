...
Este script lo resuelve:
  ...
  1. Identifica al agente por su sesión tmux (lanide_<id>).
  2. Le pregunta a LanIde QUÉ TEXTO escribió cada agente en los archivos que
  ...
  4. Toma un LOCK (flock sobre .git/lanide-commit.lock) que serializa
...
Si LanIde no responde, degrada al camino viejo (LIVE.md + archivo completo) y
te lo dice, para que mires el diff staged antes de confiar.

Uso:  python3 scripts/commit_propio.py -m "fix(x): mensaje" [ruta extra ...]

Si no corrés dentro de una terminal de LanIde, te lo dice y no hace nada
...
def trailer_atribucion(nombre, mi_tid):
    """Trailer `LanIde-Agent: <nombre> (tid N)` que hace recuperable de qué
    ...
    return f'LanIde-Agent: {quien} (tid {mi_tid})'
...
def _fragmentos_de_lanide(mi_tid):
    """{mios, ajenos, archivos} del server. None si LanIde no contesta (ahí se
    cae al camino viejo: LIVE.md + archivo completo)."""
    import json
    import urllib.request
    puerto = os.environ.get('LAN_IDE_PORT', '3000')
    try:
...
def main():
    ...
    if mi_tid is None:
        print('commit-propio: no estás en una terminal de LanIde (lanide_<id>) — '
    ...
    if not raiz:
        ...
        return 1
    try:
        with open(os.path.join(raiz, '.lan-ide', 'LIVE.md'), encoding='utf-8') as f:
            live = f.read()
    except OSError:
        live = ''
    ...
    # Provenance real (hook del CLI). Si LanIde no contesta, se sigue con
    # LIVE.md y archivo completo — el camino de antes, degradado pero vivo.
    prov = _fragmentos_de_lanide(mi_tid)
    if prov is None:
        print('commit-propio: LanIde no responde — voy con LIVE.md y archivos '
    ...
    lock_path = os.path.join(raiz, '.git', 'lanide-commit.lock')
    staged, saltados = [], []
