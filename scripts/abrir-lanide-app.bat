@echo off
REM ── LanIde — LA puerta de entrada desde Windows (post-app nativa) ──
REM
...
REM   - Repo en WSL: $HOME/lan-ide  (override: LAN_IDE_WSL_DIR)
REM   - Distro: la default de WSL            (override: LAN_IDE_WSL_DISTRO)
REM
REM Detalles:
...
if defined LAN_IDE_WSL_DISTRO (
  set "WSL=wsl.exe -d %LAN_IDE_WSL_DISTRO%"
) else (
  set "WSL=wsl.exe"
)
...
REM Repo = LAN_IDE_WSL_DIR, o $HOME/lan-ide adentro de la distro.
REM Windows reenvia LAN_IDE_WSL_DIR al entorno de WSL si esta seteada.
REM (sin comillas anidadas: cmd.exe no escapa \" como bash)
%WSL% -- bash -lc "REPO=${LAN_IDE_WSL_DIR:-$HOME/lan-ide}; test -f $REPO/scripts/reiniciar-server.sh || exit 42; cd $REPO && mkdir -p data && setsid nohup bash scripts/reiniciar-server.sh >>data/lanzador.log 2>&1 </dev/null & exit 0"
...
:sin_repo
echo No encuentro Lan Ide dentro de WSL.
echo Clonalo ahi ^(nombre publico del repo^):
echo   git clone https://github.com/rangwalaaliasgar55-bot/Lan-Ide.git ~/lan-ide
echo Si ya esta en otra ruta, setea la variable de entorno LAN_IDE_WSL_DIR
echo a esa ruta Linux ^(ej. /home/vos/mis-apps/lan-ide^).
pause
exit /b 1

:fallo
echo No pude levantar el server tras 90s. Proba a mano dentro de WSL:
echo   bash ~/lan-ide/scripts/reiniciar-server.sh
echo (log del intento: ~/lan-ide/data/lanzador.log)
echo Repo en otra ruta? setea LAN_IDE_WSL_DIR. Otra distro? setea LAN_IDE_WSL_DISTRO.
pause
exit /b 1
