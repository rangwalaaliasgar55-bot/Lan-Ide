...
server mantiene `.lan-ide/mailbox-pendientes.json` (por terminal destino); este
...
def main():
    try:
        ...
        if not raiz:
            return 0
        path = os.path.join(raiz, '.lan-ide', 'mailbox-pendientes.json')
        ...
        print(f"— Tenés {len(msgs)} mensaje(s) sin leer en .lan-ide/MAILBOX.md —")
        for l in lineas[:6]:
            print('  ' + l)
