...
Coordinarse salía carísimo. Medido sobre este proyecto:
  · `.lan-ide/MAILBOX.md` son 46 KB / 11,6K tokens y el protocolo pedía releerlo
...
Stdlib pura. Si LanIde no responde, cada comando lo dice y sale con código 1.
"""
import json
import os
import sys
...
def _base():
    return f"http://127.0.0.1:{os.environ.get('LAN_IDE_PORT', '3000')}"
...
def _tid():
    ...
    if tid:
        return tid
    # Respaldo: la sesión tmux se llama lanide_<id>.
    import re
    import subprocess
    try:
        r = subprocess.run(['tmux', 'display-message', '-p', '#{session_name}'],
                           capture_output=True, text=True, timeout=3)
        m = re.match(r'lanide_(\d+)', (r.stdout or '').strip())
        if m:
            return m.group(1)
    except Exception:
        pass
...
def main(argv):
    ...
    tid = _tid()
    if not tid:
        return _fallo('no estás en una terminal de LanIde (no encuentro '
                      'LAN_IDE_TERMINAL_ID ni la sesión tmux lanide_<id>).')
    try:
        return fn(tid, args)
    except urllib.error.HTTPError as e:
        return _fallo(f'LanIde respondió {e.code} — ¿el server está al día?')
    except Exception as e:
        return _fallo(f'no pude hablar con LanIde ({e}). ¿Está corriendo?')
