...
escáner de secretos:
  ...
  - El script identifica al agente que commitea por su sesión tmux
    (`lanide_<terminal_id>`) y lee `.lan-ide/LIVE.md`.
...
def registrar_bloqueo(raiz, mi_tid, viol):
    """Deja el bloqueo en data/lanide.log (mismo formato JSON-lines que
    ...
    try:
        ...
        data_dir = os.environ.get('LAN_IDE_DATA_DIR', '').strip() or os.path.join(raiz, 'data')
        ...
        with open(os.path.join(data_dir, 'lanide.log'), 'a', encoding='utf-8') as f:
...
def detectar_terminal_id():
    ...
    terminal de LanIde (p. ej. el usuario commiteando desde su propio shell).

    PRIMERO la variable de entorno, que LanIde inyecta al crear la terminal y
    ...
    tid = (os.environ.get('LAN_IDE_TERMINAL_ID') or '').strip()
    if tid.isdigit():
        return int(tid)
    try:
        ...
        if r.returncode == 0:
            m = re.match(r'lanide_(\d+)', r.stdout.strip())
            if m:
...
def main():
    try:
        ...
        print('✖ BLOQUEADO: estás por commitear archivos de OTRO agente (según .lan-ide/LIVE.md):')
        for v in viol:
