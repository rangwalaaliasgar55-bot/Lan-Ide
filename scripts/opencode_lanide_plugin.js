// LanIde — plugin de PROVENANCE para opencode (gemelo del hook PostToolUse de
...
// va envuelto en try/catch y el fetch es fire-and-forget con timeout: si LanIde
// está caído o lento, el agente sigue igual.
//
// Lo instala LanIde al boot (plotspace/core/cli_adapters.py) en
// ~/.config/opencode/plugin/ — un solo archivo cubre a TODOS los agentes
// opencode que se lancen en tmux. La identidad viene de LAN_IDE_TERMINAL_ID, que
// LanIde inyecta en el entorno de cada pane (terminals.py); sin esa var (opencode
// fuera de LanIde) el plugin es un no-op.
import { readFile } from "node:fs/promises";

const TID = process.env.LAN_IDE_TERMINAL_ID;
const PORT = process.env.LAN_IDE_PORT || "3000";
...
async function post(tool_name, tool_input, cwd) {
  try {
    ...
    return await r.json();
  } catch { return null; }       // LanIde caído/lento: el agente sigue igual
}
...
function inyectar(salida, texto) {
  try {
    ...
    if (typeof salida.output === "string") salida.output += `\n\n${texto}`;
      ...
      ...(salida.metadata || {}), lanide: texto };
  } catch { /* nunca romper */ }
}

export const LanIdeSwarm = async ({ directory, worktree }) => {
  if (!TID) return {};                       // opencode fuera de LanIde → no-op
