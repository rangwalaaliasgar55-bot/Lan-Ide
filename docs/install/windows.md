...
## Downloadable setup EXE

Download **LanIdeSetup.exe** from the latest [Lan Ide GitHub Release](https://github.com/rangwalaaliasgar55-bot/Lan-Ide/releases/latest) and run it. The setup bootstraps WSL2 Ubuntu if needed, installs Lan Ide into `~/lan-ide`, and creates **LanIde.bat** on the Desktop.

Lan Ide is a **Linux** app (Python + uvicorn + **tmux**). On Windows
the engine runs inside **WSL2 Ubuntu**. If you prefer the script installer, one command does the rest:
...
the same line again. After it finishes you get **LanIde.bat** on the Desktop —
...
⚙ → **Accounts**. Lan Ide does not ship those products.
...
```bash
git clone https://github.com/rangwalaaliasgar55-bot/Lan-Ide ~/lan-ide
cd ~/lan-ide
```

If the repo already lives somewhere else, set `LAN_IDE_WSL_DIR` (Linux path) before
...
# Optional: install the `lanide` CLI entry point
pip install -e .
...
```bash
source ~/lan-ide/venv/bin/activate
lanide
...
- **Desktop launcher (what `install.ps1` copies):** `scripts/abrir-lanide-app.bat`
  as `LanIde.bat` on the Desktop. It warms WSL, starts the server if needed
  (`~/lan-ide/scripts/reiniciar-server.sh`), and opens Chrome in app
  mode at `http://localhost:3000`.
- **Compiled window (optional):** from WSL, `bash scripts/compilar-lanzador-windows.sh`
  builds a thin WebView2 window (`LanIde.exe`) around the same Linux server.
...
| | WSL2 path | Docker |
|---|---|---|
| Code / engine | `~/lan-ide` | image + bind mounts |
| App data (DB, CLI account secrets) | `<repo>/data` by default, or `LAN_IDE_DATA_DIR` (e.g. `~/.local/share/lanide`) | `./data` → `/app/data` |
| Your projects | anywhere under the Linux FS (prefer `~/proyectos`, not `/mnt/c`) | host folder from `PROYECTOS_DIR` → `/proyectos` |

## What you still need

- **Your agent CLIs** installed and logged in (Lan Ide does not redistribute them).
