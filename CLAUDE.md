...
- **Before editing a file another agent may have touched**, look at its history: `git log --oneline -- <file>` and `git show <commit>`. If there's a recent commit, understand what was done and why, and build ON TOP without undoing it. That's the "past" layer complementing the live layer (`.lan-ide/LIVE.md` = who touches what NOW).
...
- **Hard enforcement (ownership lock):** this rule is not just discipline —
  ...
  the agent by its tmux session (`lanide_<id>`) and BLOCKS the commit if among the staged
...
- **Going to edit a key file with no owner? RESERVE IT first** (the queue forms before the
  clash): `- @You -> @lanide: RESERVE <file> — what you'll do` in the MAILBOX (15 min read; see the LIVE protocol).
...
# scripts/reiniciar-server.sh is a USER tool (or LanIde's own), not agents'. And NEVER pkill -f uvicorn + relaunch.
...
- `data/` — local state (gitignored). `.workspace/` is a per-project artifact (gitignored): `STATE.md` (written by LanIde every 10s, read by agents) + per-terminal logs in `.workspace/logs/terminal_{id}_{name}.log` (useful for debugging terminals).
...
- **Terminal layout** (`sections/terminals/terminal-layout.js`): modes **mosaic** (default) ⇄ **free/vertical** (drag + resize by 4 corners), min 280×160px per card, fixed 13px font. The xterm canvas ALWAYS follows its card (only freeze under 60×40px); whoever changes sizes fires `TerminalLayout.relayoutAll()` / `LanIdeEditor.relayout()`.
...
- **Shortcuts:** Ctrl+B strip · Ctrl+T new project · Ctrl+\\ quick terminal · Ctrl+P dock (file palette if the editor is visible; Ctrl+Shift+P command palette) · Ctrl+E editor · Ctrl+J lanide · Ctrl+K search project · Ctrl+1…9 jump to project N · Esc closes/de-maximizes. PTT voice configurable (default: hold AltLeft).
...
- **Layout de terminales** (`sections/terminals/terminal-layout.js`): modos **mosaico** (default) ⇄ **libre/vertical** (drag + resize por 4 esquinas), mínimo 280×160px por card, fuente fija 13px. El canvas xterm SIEMPRE sigue a su card (único freeze bajo 60×40px); quien cambia tamaños dispara `TerminalLayout.relayoutAll()` / `LanIdeEditor.relayout()`. Ver [[terminales-modo-libre]].
...
- `core/logs.py` — swarm audit trail in JSON-lines (`data/lanide.log`, rotates at 5MB). `core/mantenimiento.py` — janitor: purges `.workspace/logs` every 30 min + old `task_events` at boot.
...
- **`hay_update` = there is a NEW COMMIT since boot** (HEAD moved). Uncommitted edits don't light it. "Update now" banner at the strip bottom; **it does NOT hide while agents work** (`agentes_trabajando` from `/version` is informational, scope = LanIde project).
- **Who updates: THE USER** (click) or LanIde itself — agents verify with pytest + import smoke and do NOT restart the server.
...
The monitor distinguishes the TASK_DONE LanIde sent as instruction from the agent's real TASK_DONE with 3 layers: `_ANSI_RE` (strips ANSI) → `_KW_SOLO_RE` (`^[^a-zA-Z]*TASK_DONE[^a-zA-Z]*$`, no letters around) → baseline at monitor start (ignores the pane's prior history). The sentinel-file is the primary closure source today; this stays as fallback.
...
- Session per terminal: `lanide_{terminal_id}`. Create: `subprocess.run(['tmux','new-session','-d',...])` + `mouse on`, `focus-events off`, `window-size latest`, `status off` (and `CODEX_HOME` of the active account for Codex).
...
Without reloading the page: disconnect the terminal WSs → `history.pushState` → `cargarProyecto()`. On project change, `onProjectChanged(projectId)` is notified to LanIdeEditor, TerminalLayout, LanIdeTasks, LanIdeMemory, LanIdeReview and LanIdeDock (the last restores the target project's persisted state/tab).
...
- **Infra:** `LAN_IDE_ALLOWED_HOSTS` · `LAN_IDE_HOST` / `LAN_IDE_PORT` · `AUTO_PUSH` (on — fe_watch pushes `master` to origin when it detects commits; it's the automatic backup, don't push by hand)

<!-- LAN_IDE_SKILLS_START -->
...
Skill qa-browser-lanide — ver `.claude/skills/qa-browser-lanide.md` (cómo verificar en browser + correr los tests)
...
<!-- LAN_IDE_MEMORY_START -->
## 🧠 Shared project memory (LanIde)

This project has a shared memory between ALL agents in `.lan-ide/memory/`. **Before starting any task**: search `.lan-ide/memory/INDEX.md` for the memories that touch your task and open them. Do NOT read the whole INDEX (it's ~7K tokens and grows): GREP it for your topics — `grep -i "terminal\|xterm" .lan-ide/memory/INDEX.md` — or read only the category section (it's grouped; each line carries title, #tags and date). Reading it all is only worth it for a cross-cutting task.
...
1. Create `.lan-ide/memory/<kebab-case-slug>.md` with this exact format:
   ...
   `terminales` (Terminals & tmux) · `ui` (UI · Workspace) · `swarm` (Backend & Swarm) · `diseno` (Design & Craft) · `preview` (Web Preview & Radio) · `cuentas` (Accounts & CLIs) · `voz` (Voice & Audio) · `desktop` (Desktop) · `entorno` (Environment · WSL & Git) · `producto` (Product & Roadmap). If unsure, LanIde infers it from the tags; a memory that fits none is flagged `sin-clasificar` in the health check.

2. The INDEX (`.lan-ide/memory/INDEX.md`) is regenerated by LanIde alone, GROUPED by category and enriched — don't edit it by hand.
...
TASK CLOSURE (with or without workflow) — when finishing ANY task, signal closure; it's the telemetry this memory learns from (what worked, what failed). Run:
    ...
    TID=${LAN_IDE_TERMINAL_ID:-$(tmux display-message -p '#S' 2>/dev/null | sed 's/^lanide_//')} && mkdir -p .lan-ide/signals && printf '%s' '{"estado":"done","motivo":"","memorias_usadas":[]}' > .lan-ide/signals/terminal_${TID}.json

In `memorias_usadas` list the slugs from `.lan-ide/memory/` you read and used ([] if none). If you end up `blocked`/`error`, `motivo` is MANDATORY and concrete; in `done` it's optional (one line with the non-obvious approach that worked). In workflows the engine already gives you this instruction with your id — no need to figure it out then.
<!-- LAN_IDE_MEMORY_END -->
...
<!-- LAN_IDE_LIVE_START -->
## 🔴 Swarm coordination — `.lan-ide/jv`

You work with other agents on the SAME tree. Don't read state files to find out: ask when you need to.

    .lan-ide/jv estado      what the others are touching and what arrived for you
    .lan-ide/jv inbox       your new messages (do NOT read .lan-ide/MAILBOX.md whole)
    .lan-ide/jv msg "<agent>" "<text>"    leave a notice (does NOT interrupt)
    .lan-ide/jv ask "<agent>" "<question>"  ask and WAIT for the answer
    .lan-ide/jv claim "<symbol|file|folder>"  reserve YOUR zone
    .lan-ide/jv commit -m "<message>"      commit only YOUR work, by hunk
...
2. **Never rewrite a whole file** that isn't yours (`Write` over something existing). Edit by zone: two agents in different zones of the same file coexist fine, but a full overwrite with your stale copy erases the other's work without a trace. LanIde stops you if it happens.
3. **Don't delete or rename what another agent claimed.** LanIde stops you before writing, naming the owner. If it really must go, leave them a `jv msg` and let them adapt it — yanking it out breaks their code without them knowing. (Using their function is fine; nobody stops you.)
...
`.lan-ide/LIVE.md` still exists (who owns what, permissions, reservations) if you want the detail, but `jv estado` gives you what you need.
<!-- LAN_IDE_LIVE_END -->

<!-- LAN_IDE_LECCIONES_START -->
## 📚 Swarm lessons (always loaded — from real failures and findings)


This block is generated by the app from your local swarm memory (`.lan-ide/memory/lecciones-del-enjambre.md`): it distills the mistakes and findings of *your own* agents in ≤20 short rules, and refreshes on every session. Don't edit between markers — write a `[leccion]` memory instead.<!-- LAN_IDE_LECCIONES_END -->
