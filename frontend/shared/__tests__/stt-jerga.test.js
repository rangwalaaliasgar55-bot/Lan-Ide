// Tests de LanIdeSTT (stt-jerga.js): post-corrección determinista de jerga del
...
const { corregirJerga, diagnosticoMic } = globalThis.LanIdeSTT._pure;

// ── corregirJerga: los errores REALES observados (benchmark 2026-07-09) ──
assert.strictEqual(
  corregirJerga('LanIde, hace que mid del frontend'),
  'LanIde, hace commit del frontend',
...
assert.strictEqual(corregirJerga('abrí una terminal con yarvis'), 'abrí una terminal con LanIde');
assert.strictEqual(corregirJerga('preguntale a jarbis'), 'preguntale a LanIde');
...
for (const muerta of ['decidirFuenteDictado', 'elegirAlternativa', 'unirFragmento',
  ...
  assert.ok(!(muerta in globalThis.LanIdeSTT), `API muerta expuesta: ${muerta}`);
  assert.ok(!(muerta in globalThis.LanIdeSTT._pure), `API muerta en _pure: ${muerta}`);
}
