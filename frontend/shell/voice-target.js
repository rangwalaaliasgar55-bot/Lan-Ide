...
// Lógica PURA (sin DOM) de "¿a qué terminal —o a LanIde— va este dictado?".
...
(function (root) {
  ...
  //   lanideVisible   — el dock está abierto en la pestaña 'lanide'
  ...
  //   focoLanIde      — el foco del teclado está en el composer de LanIde
  // Devuelve el destino o null (sin destino: el caller avisa y no graba).
  function resolverDestinoVoz({ proyectoAbierto, lanideVisible, fijado, hover,
                                terminales, activaId, focoLanIde }) {
    ...
    if (focoLanIde) return { type: 'lanide' };
    if (hover?.type === 'lanide') return { type: 'lanide' };
    ...
    // 4. Fijado en LanIde (o terminal que murió). Con el chat a la vista, suyo.
    if (lanideVisible) return { type: 'lanide' };

    // 5. LanIde oculto: la voz se la queda una terminal.
    if (vive(activaId)) return { type: 'terminal', id: activaId };
    // Pantalla de arranque (ninguna terminal): hablar al aire ES hablarle a
    // LanIde — pedís "creame dos terminales" y el orquestador lo ejecuta sin
    // abrir el dock. `manosLibres` = el usuario no está mirando el chat.
    if (!vivas.length) return { type: 'lanide', manosLibres: true };
