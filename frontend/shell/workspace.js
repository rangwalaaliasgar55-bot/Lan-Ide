// LAN_IDE — Workspace: terminales + orquestador + sidebar de proyectos
...
// Las otras cards del "Arranque" (rediseño 2026-07-10): LanIde / Editor.
document.getElementById('tea-card-lanide')?.addEventListener('click', () => window.LanIdeDock?.setTab?.('lanide'));
...
// lanide.control.<id> con shape {type:'key'|'mouse', value}.
(function _teaKeycaps() {
  const label = (id, fallback) => {
    try {
      const b = JSON.parse(localStorage.getItem(`lanide.control.${id}`) || 'null');
...
async function _abrirEditorWorkspace() {
  const ed = window.LanIdeSlideEditor;
  if (!ed) return;
  ...
  if (!pick) { toast(window.LanIdeI18n?.t?.('El proyecto no tiene archivos todavía') ?? 'El proyecto no tiene archivos todavía', 'info'); return; }
...
const _panel  = () => window.lanidePanel;
...
function _focoOcupadoEnOtroLado() {
  ...
  if (a === _panelTA()) return false;        // ya está en el composer de LanIde
...
// usuario lo sentía como "el mic tarda ~1s en escucharme" en LanIde).
...
// apps con LanIde abierto: bajar de nuevo y buscar otra vía (este comentario
...
// ─── Estado de los paneles: ahora vive en window.LanIdeDock ──────────
...
Object.defineProperty(window, 'editorVisible', {
  get: () => window.LanIdeDock?.activeTab?.() === 'editor',
  configurable: true,
});

// ─── Estado del editor Monaco ───────────────────────────────────────
// MOVIDO a frontend/js/editor.js (window.LanIdeEditor). workspace.js ya no
...
let _voiceTarget       = { type: 'lanide' };
let _activeVoiceSession = null;
let _hoverVoz          = null;   // {type:'terminal',id} | {type:'lanide'} | null

function _resolveVoiceTarget() {
  const a = document.activeElement;
  return window.VoiceTarget.resolverDestinoVoz({
    proyectoAbierto: !!projectId,
    // LanIde "visible" = dock abierto en la pestaña 'lanide' (con LanIde oculto
    // la voz se la queda una terminal — bug histórico "LanIde oculto seguía
    // recibiendo").
    lanideVisible: window.LanIdeDock?.activeTab?.() === 'lanide',
    fijado:        _voiceTarget,
    hover:         _hoverVoz,
    terminales:    [...terminales.keys()],
    activaId:      _idCardActiva(),
    // Foco del teclado en el composer de LanIde: estás escribiendo ahí, el
    // dictado anexa a ese texto aunque el mouse descanse sobre una terminal.
    focoLanIde:    !!a && (a === _panelTA() || !!a.closest?.('#orch-panel')),
  });
}
...
// Click en el panel del orquestador → target = lanide.
...
document.addEventListener('pointerover', (e) => {
  ...
  else if (e.target.closest?.('#orch-panel')) _setHoverVoz({ type: 'lanide' });
...
// 'editable' (composer de LanIde, editor, inputs, iframes) es intocable: lo que
...
function _voiceTargetLabel(session) {
  if (!session) return 'LanIde';
  if (session.type === 'lanide') return 'LanIde';
...
function _renderMensaje(rol, texto, extras) {
  _panel()?.addMessage({
    ...
    role:      rol === 'lanide' ? 'lanide' : 'user',
    author:    rol === 'lanide' ? 'LanIde' : 'Tú',
    badge:     rol === 'lanide' ? 'orchestrator' : undefined,
    timestamp: new Date(),
    content:   texto,
    ...(extras || {}),
  });
...
function conectarEventosWs() {
  ...
  eventsWs.onmessage = e => {
    ...
    if (data.type === 'hola') {
      if (_bootIdServer && data.boot_id !== _bootIdServer) {
        ...
        if (window.LanIdeUpdater?.recargarCuandoListo) {
          window.LanIdeUpdater.recargarCuandoListo();
        } else {
          if (window.LanIdeUpdater) window.LanIdeUpdater.recargando = true;
          location.reload();
        }
        return;
      }
    ...
    if (data.type === 'task_event') {
      ...
      if (data.terminal_id != null) {
        if (data.event === 'TASK_DONE') {
          ...
          window.LanIdeDock?.notify?.('review', 1);
      ...
      window.LanIdeTasks?.onTasksUpdate?.();
      window.LanIdeDock?.notify?.('tasks', 1);
      ...
      window.LanIdeTasks?.onWorkflowUpdate?.(data);
      window.LanIdeDock?.notify?.('tasks', 1);
      ...
      agregarMensajeChat('lanide', data.message);
      window.LanIdeDock?.notify?.('lanide', 1);
      ...
      if (!refrescada) window.LanIdeDock?.notify?.('preview', 1);
      window.LanIdeDevServers?.refrescar?.();
      ...
      window.LanIdeNotify?.avisar?.({ tipo: 'termino', nombre: _nombreTerm(data.terminal_id), sonidoOn: sonidoTareas });
      ...
      window.LanIdeNotify?.avisar?.({ tipo: 'espera', nombre: _nombreTerm(data.terminal_id), sonidoOn: sonidoTareas });
      _pingTitulos();
      ...
      window.LanIdeMemory?.onLiveEvent?.(data);
      ...
      window.LanIdeSettings?.onCuentaAgregada?.(data);
      window.LanIdeSettings?.refrescar?.('cuentas');
  ...
  eventsWs.onclose  = (event) => {
    ...
    window.LanIdeUpdater?.avisarPosibleCaida?.();
...
async function consultarMobilePreview(autoAbrir = false) {
  ...
  try {
    ...
    if (!d.es_expo) {
      ...
      window.LanIdeDock?.setTabVisible?.('mobile', false);
    ...
    window.LanIdeDock?.setTabVisible?.('mobile', true);
    ...
    const autoMostrar = localStorage.getItem('lanide.autoMobilePreview') !== '0';
    const enMobile = window.LanIdeDock?.isOpen?.() && window.LanIdeDock?.activeTab?.() === 'mobile';
    const vistoKey = `lanide.mobilePreview.visto.${pid}`;
    ...
    if (dec.abrir) window.LanIdeDock?.notify?.('mobile');
  } catch {
...
let sonidoTareas = localStorage.getItem('lanide.sonidoTareas') !== 'off';  // default ON
let _audioCtx = null;
...
document.getElementById('terminals-sound-btn')?.addEventListener('click', () => {
  ...
  localStorage.setItem('lanide.sonidoTareas', sonidoTareas ? 'on' : 'off');
...
function _restaurarChat() {
  ...
  if (sesion?.length > 0) {
    p.setMessages(sesion.map((m, i) => ({
      ...
      role:      m.rol === 'lanide' ? 'lanide' : 'user',
      author:    m.rol === 'lanide' ? 'LanIde' : 'Tú',
      badge:     m.rol === 'lanide' ? 'orchestrator' : undefined,
      timestamp: new Date(),
      content:   m.texto,
    })));
  } else {
    p.setMessages([{
      ...
      role: 'lanide', author: 'LanIde', badge: 'orchestrator',
      timestamp: new Date(),
...
// Detecta el idioma de un texto del USUARIO (es|en) para que la VOZ de LanIde
...
async function reproducirVoz(texto) {
  try {
    ...
    const lang = window._orchLangConversacion || window.LanIdeI18n?.lang?.() || 'es';
...
async function reproducirBienvenida() {
  try {
    ...
    const lang = window.LanIdeI18n?.lang?.() || 'es';
...
if (_bootstrapNuevo && !projectId) {
  window.addEventListener('DOMContentLoaded', () => {
    ...
    const _saludo = () => { if (elTitulo) elTitulo.textContent = window.LanIdeI18n?.t?.('Bienvenido') ?? 'Bienvenido'; };
    _saludo();
    window.addEventListener('lanide:lang', _saludo);  // #project-title es zona i18n-skip
...
async function inicializar() {
  try {
    ...
    window.LanIdeDock?.init({
      onTabShown: (tab) => _onDockTabShown(tab),
      onResized: () => {
        if (typeof terminales !== 'undefined') terminales.forEach((_, id) => refitTerminal?.(id));
        if (window.LanIdeDock?.activeTab?.() === 'editor') window.LanIdeEditor?.relayout?.();
      },
    });
    window.LanIdeDock?.setProject(projectId);

    await Promise.all([cargarProyecto(), cargarSidebar()]);

    window.LanIdeTasks?.init(projectId);
    window.LanIdeMemory?.init(projectId);
    window.LanIdeReview?.init(projectId);
    window.LanIdeSettings?.init(projectId);
    document.getElementById('jw-gear')?.addEventListener('click', () => window.LanIdeSettings?.open());
    window.LanIdeGroqSetup?.init?.();
    ...
    window._orchOnMicRelease = () => {
      ...
      // dictado terminó ACÁ — la esfera de LanIde pasa a "cargando" y la
      ...
      _pttProcesando();
      if (_activeVoiceSession?.type === 'lanide') setEstado('processing');
    ...
    window._orchOnMicDiscard = () => {
      ...
      _detenerWaveform();
      window.LanIdeVoiceField?.apagar?.();
      _activeVoiceSession = null;
      if (window.LanIdeDock?.activeTab?.() === 'lanide') setEstado('idle');
    ...
    // window.LanIdeClisEstado para que picker/launcher pinten el aviso
    ...
    window.LanIdeDevServers?.init();
    window.LanIdeDevServers?.cargar?.(projectId);

    // Inicializar el editor (window.LanIdeEditor) con el proyecto actual.
    ...
    window.LanIdeEditor?.init(projectId);
...
async function cargarProyecto() {
  ...
  document.title = `LAN_IDE — ${estado.project.nombre}`;
...
const _SB_FICHAS_LS = 'lanide.fichasGrandes';
...
const _SB_RECLAMOS_LS = 'lanide.reclamos';
...
const _sbT = (s) => window.LanIdeI18n?.t?.(s) ?? s;
...
function renderSidebar() {
  ...
  const _t = window.LanIdeI18n?.t || ((s) => s);
...
// deslizante al cerrarse (window.LanIdeStripFiles.colapsar), así el "cajón" de
...
window.LanIdeStripFiles = { colapsar: _sbColapsarArboles };
...
async function _sbEliminar(id, path, tipo, name) {
  ...
  try {
    ...
    if (!esDir) window.LanIdeSlideEditor?.cerrarArchivo?.(path);   // si estaba abierto en el editor, cerrar su pestaña
...
function _sbTreeChildrenHTML(nodes, depth, protegido, creados) {
  ...
  // En un proyecto protegido (LanIde) solo se puede borrar lo creado desde el editor (set `creados`).
...
function _sbWireTree(scope, projId) {
  ...
  scope.querySelectorAll('.sb-ffile').forEach(el => {
    ...
    if (window.LanIdeSlideEditor?.isDirty?.(el.dataset.path)) el.classList.add('jw-dirty');
    if (el.dataset.wired) return; el.dataset.wired = '1';
...
async function _sbAbrirArchivo(projId, path) {
  try {
    ...
    window.LanIdeSlideEditor?.abrirArchivo?.(projId, path);
  } catch {}
}
...
window.addEventListener('lanide:lang', () => renderSidebar());
...
document.addEventListener('keydown', (e) => {
  ...
  if ((e.key === 'p' || e.key === 'P') && !e.altKey) {
    ...
    if (e.shiftKey && window.LanIdeEditor?.openPalette) {
      window.LanIdeEditor.openPalette('comando');
    } else if (window.LanIdeDock?.isOpen() && window.LanIdeDock.activeTab() === 'editor'
               && window.LanIdeEditor?.openPalette) {
      window.LanIdeEditor.openPalette('archivo');
    } else {
      window.LanIdeDock?.toggle();
    }
    return;
  }
  ...
  if ((e.ctrlKey || e.metaKey) && !e.shiftKey && !e.altKey) {
    ...
    if (k === 'j' && !_editando) { e.preventDefault(); window.LanIdeDock?.setTab?.('lanide'); return; }
...
document.addEventListener('keydown', (e) => {
  if (e.key === 'Escape') {
    if (window.LanIdeDock?.isMaximized?.()) { window.LanIdeDock.setMaximized(false); return; }
  }
});
...
async function cambiarProyecto(nuevoId) {
  ...
  const _descartarOk = await window.LanIdeEditor?.confirmarDescarteSiSucio?.();
  ...
  window.LanIdeEditor?.onProjectChanged(projectId);
  window.LanIdeSlideEditor?.onProjectChanged?.(projectId);
  window.TerminalLayout?.onProjectChanged(projectId);
  window.LanIdeTasks?.onProjectChanged(projectId);
  window.LanIdeMemory?.onProjectChanged(projectId);
  window.LanIdeReview?.onProjectChanged(projectId);
  window.LanIdeSettings?.onProjectChanged(projectId);
  window.WebPreview?.onProjectChanged?.(projectId);
  ...
  window.LanIdeDock?.onProjectChanged(projectId);
  ...
  if (window.LanIdeDock && window.LanIdeDock.isOpen?.() && window.LanIdeDock.activeTab?.() === 'preview') {
    window.WebPreview?.init?.(document.getElementById('jw-pane-preview'));
  ...
  try {
    ...
    agregarMensajeChat('lanide', `No pude cargar el proyecto (${err.message})`);
  }
...
window.addEventListener('popstate', async e => {
  ...
  if (id && id !== String(projectId)) {
    ...
    const _popDescartarOk = await window.LanIdeEditor?.confirmarDescarteSiSucio?.();
...
async function iniciarGrabacion() {
  if (window.LanIdeGroqSetup?.haceFaltaClave?.()) {
    window.LanIdeGroqSetup.abrir();
    return;
  }
  _micSoltado = false;
  ...
  //    sin tocar nada (esto arregla el bug de "LanIde oculto seguía recibiendo").
  ...
  if (!target) {
    ...
    _toastWarn(projectId ? 'Poné el mouse sobre una terminal para hablarle'
                         : 'Abrí un workspace para hablarle a LanIde');
    return;
  }
  ...
  if (_sinTerminales()) window.LanIdeVoiceField?.escuchar?.();
  ...
  try { fetch('/api/voice/prewarm', { method: 'POST' }).catch(() => {}); } catch {}
  // Snapshot de lo que el usuario ya escribió en el textarea de LanIde: el
  // dictado se ANEXA a este prefijo en vez de pisarlo.
  if (target.type === 'lanide' && !target.manosLibres) {
  ...
  try {
    ...
    if (target.type === 'lanide' && window.LanIdeDock?.activeTab?.() === 'lanide') {
      agregarMensajeChat('lanide', 'Sin acceso al micrófono. Revisá los permisos del browser.');
    } else {
  ...
  // Solo cambiamos la esfera si la voz va a LanIde. Si va a una terminal,
  // el toast de PTT es feedback suficiente y la esfera se mantiene neutral.
  if (target.type === 'lanide') setEstado('recording');
}
...
// commit, "yarvis" → LanIde…); vive en shared/stt-jerga.js. Fallback inocuo
// si el módulo no cargó.
const _corregirJerga = t => window.LanIdeSTT?.corregirJerga?.(t) ?? t;
...
async function procesarAudio(gen = _micGen) {
  ...
  const diagMic = window.LanIdeSTT?.diagnosticoMic?.({
  ...
  // perdió (race), default a lanide SOLO si está visible.
  const session = _activeVoiceSession || (window.LanIdeDock?.activeTab?.() === 'lanide' ? { type: 'lanide' } : null);
  _activeVoiceSession = null;

  if (!session) { _cancelarHoldsActivos(); return; }

  if (session.type === 'lanide') setEstado('processing');
  ...
  try {
    ...
    if (!text) {
      ...
      if (session.type === 'lanide' && !session.manosLibres) agregarMensajeChat('lanide', _avisoMic(diagMic) || '(No te entendí, intentá de nuevo)');
    ...
    // lanide MANOS LIBRES: hablaste al aire (sin el chat a la vista). El dictado
    // NO pasa por el textarea — se manda derecho al orquestador y el dock se
    // abre en LanIde para que veas la respuesta y lo que va haciendo. Tocar el
    // textarea acá pisaría un borrador que el usuario ni está viendo, y lo
    // auto-enviaría de arrastre.
    if (session.manosLibres) {
      window.LanIdeDock?.open?.('lanide');
      _voiceTarget = { type: 'lanide' };   // el chat ya está a la vista: el próximo dictado sigue siendo suyo
    ...
    // lanide — anexar el dictado a lo que el usuario ya había escrito (prefijo)
    ...
    if (ta) {
      ...
      // "Auto-enviar a LanIde": manda el mensaje al toque, sin tener que apretar ↵.
    ...
    if (session.type === 'lanide') agregarMensajeChat('lanide', `Error transcribiendo: ${err.message}`);
    ...
    if (session.type === 'lanide' && gen === _micGen) setEstado('idle');
  }
}
...
async function enviarMensaje(texto, imagenBase64 = null, mediaType = null) {
  ...
  // Recordá en qué idioma ESCRIBE el usuario → la VOZ de LanIde le responde en
  ...
  const historial = (chatSesiones[pid] || []).slice(-12).map(m => ({
    role: m.rol === 'lanide' ? 'assistant' : 'user',
  ...
  try {
    ...
    try {
      ...
      if (pid === projectId) agregarMensajeChat('lanide', data.response);
      else { (chatSesiones[pid] ||= []).push({ rol: 'lanide', texto: data.response }); }
    }
    ...
    if (pid === projectId) agregarMensajeChat('lanide', `Error: ${err.message}`);
  } finally {
    setEstado('idle');
...
async function _chatStream(body, pid) {
  ...
  const mostrar = (t) => {
    ...
    if (!burbuja) { _renderMensaje('lanide', t, { id: msgId }); burbuja = true; }
  ...
  (chatSesiones[pid] ||= []).push({ rol: 'lanide', texto: done.response || acumulado });
  return done;
}
...
function toggleMaximizarTerminal(id) {
  ...
  if (terminalMaximizadaId === id) {
    ...
    window.LanIdeDock?.exitAgentFullscreen?.();
  } else {
    ...
    window.LanIdeDock?.enterAgentFullscreen?.(id);
...
async function _saltarLocalhostDeTerminal(id, vigente) {
  try {
    ...
    window.LanIdeDock?.revealTab?.('preview');
...
document.addEventListener('click', (e) => {
  ...
  if (!window.LanIdeDock?.isOpen?.()) return;
  const id = parseInt(card.id.replace('terminal-card-', ''), 10);
  if (!Number.isFinite(id)) return;
  _saltarLocalhostDeTerminal(id, () =>
    terminalMaximizadaId == null
    && window.LanIdeDock?.isOpen?.()
...
async function eliminarTerminal(id) {
  ...
  window.LanIdeDock?.forgetAgentFullscreen?.(id);
...
// Con la UI en inglés el título se muestra TRADUCIDO (LanIdeTitulosI18n:
...
async function _actualizarTitulosVivos() {
  ...
  for (const [tid, titulo] of Object.entries(data.titles || {})) {
    ...
    if (titulo) {
      const visible = window.LanIdeTitulosI18n ? LanIdeTitulosI18n.mostrar(titulo) : titulo;
  ...
  window.LanIdeTitulosI18n?.pedir(Object.values(data.titles || {}), _pingTitulos);
...
window.addEventListener('lanide:lang', () => _pingTitulos());
let _titDeb = 0;
...
// Lógica pura en launcher-state.js (LanIdeLauncherState); render aquí.
// Persistencia: lanide.launcher.rutas (últimas rutas lanzadas) en localStorage.
...
const _TL_RUTAS_KEY = 'lanide.launcher.rutas';
...
function _tlSetCount(tipo, n) {
  ...
  _tlCounts[tipo] = window.LanIdeLauncherState.clampContador(n, terminales.size, _tlCounts, tipo);
  _tlSync();
}
...
function _tlEstadoFaltantes() {
  ...
  if (!_tlEstadoClis) {
    try {
      const prev = JSON.parse(localStorage.getItem('lanide_clis_estado') || 'null');
      if (prev && Array.isArray(prev.clis)) {
        _tlEstadoClis = prev;
        window.LanIdeClisEstado = prev;        // cache compartida con QuickPicker
        _tlPintarFalta();
      }
  ...
  fetch('/api/clis')
    ...
    .then(d => {
      if (!d) return;
      _tlEstadoClis = d;
      window.LanIdeClisEstado = d;             // cache compartida con QuickPicker
      try { localStorage.setItem('lanide_clis_estado', JSON.stringify(d)); } catch { /* cuota/privacidad */ }
      _tlPintarFalta();
    })
    .catch(() => {});
}
...
function _tlRenderGrid() {
  const L = window.LanIdeLauncherState;
...
// ── Rutas base de ESTA máquina (hogar del usuario, no el de LanIde) ──
...
function abrirLauncher(opts = {}) {
  ...
  Object.assign(_tlCounts, window.LanIdeLauncherState.countsIniciales());
...
function _quickCrearTerminal(counts, preset) {
  const lote = window.LanIdeLauncherState.loteDesdeContadores(counts || {}, terminales.size);
...
function _abrirQuickPicker() {
  const max = window.LanIdeLauncherState?.MAX_TERMINALES ?? 12;
...
async function _tlLanzar() {
  ...
  try {
    ...
    const lote = window.LanIdeLauncherState.loteDesdeContadores(_tlCounts, esOtro ? 0 : terminales.size);
    if (lote.length > 0) {
...
const _STRIP_KEY = 'lanide.strip.hidden';   // valores: '1' (oculta) / '0' (visible)
...
function toggleStrip(action = 'toggle') {
  ...
  const oculta = window.LanIdeStrip.nextStripHidden(actual, action);
...
// onTabShown: lo invoca LanIdeDock cuando una pestaña se vuelve visible.
...
function _onDockTabShown(tab) {
  ...
  if (tab !== 'editor') window.LanIdeEditor?.detenerPolling?.();
  switch (tab) {
    case 'editor':
      // Arranca el editor si es la primera vez (lee window.editorVisible=true ahora).
      window.LanIdeEditor?.refrescarArbol();
      window.LanIdeEditor?.iniciarPolling();
      window.LanIdeEditor?.cargarMonaco();
      window.LanIdeEditor?.relayout?.();
      break;
    case 'lanide': {
      ...
      const p = window.lanidePanel;
      if (p?.$messages && !p._userScrolled) p.$messages.scrollTop = p.$messages.scrollHeight;
      break;
    }
    case 'tasks':
      window.LanIdeTasks?.show?.();
      break;
    case 'review':
      window.LanIdeReview?.mostrarEnPane?.();   // Task 26 lo define
      break;
    case 'preview': {
...
window.togglePanel = (panel) => {
  const MAP = { editor: 'editor', orquestador: 'lanide',
                tasks: 'tasks', mobile: 'mobile' };
  const tab = MAP[panel];
  if (!tab || !window.LanIdeDock) return;
  if (window.LanIdeDock.activeTab?.() === tab) window.LanIdeDock.close();
  else window.LanIdeDock.setTab(tab);
};
...
const icoPlugin = (p) =>
  window.LanIdePluginIcons?.iconoDePlugin?.(p.full_id, p.nombre) || 'plug';
...
// El listener de #ps-btn-new-skill se cablea en LanIdeSkills.montar()
...
window._orchOnHeaderAction = async (action) => {
  switch (action) {
    ...
    case 'workflows':     return window.LanIdeSettings?.open('workflows');
...
async function guardarThreadActual() {
  const mensajes = window.lanidePanel?.getMessages?.() || [];
...
async function nuevoThread() {
  ...
  window.lanidePanel?.clearMessages?.();
  if (chatSesiones[projectId]) chatSesiones[projectId] = [];
  currentThreadId = (crypto.randomUUID?.() ?? `t-${Date.now()}`);
  agregarMensajeChat('lanide', '¿Qué hacemos, señor?');
}
...
async function cargarThread(threadId) {
  try {
    ...
    for (const m of (data.mensajes || [])) {
      const rol = m.rol === 'lanide' ? 'lanide' : 'user';
...
function exportarConversacion() {
  ...
  const lineas = [`# Conversación LAN_IDE — ${proyectoNombre} — ${fechaHumana}`, ''];
  for (const m of mensajes) {
    const autor = m.role === 'lanide' ? 'LAN_IDE' : 'Usuario';
  ...
  a.href     = url;
  a.download = `lanide-${slug}-${fechaISO}.md`;
...
// ─── Bridge: workflows del proyecto (reutilizable desde LanIdeSettings) ─

window.LanIdeWorkflows = {
  async render(cont) {
    if (!cont) return;
    cont.innerHTML = '<div class="hist-empty" aria-hidden="true"><div class="skeleton"></div><div class="skeleton"></div><div class="skeleton"></div></div>';
    try {
...
const CONTROLS = [
  {
    id: 'mic-ptt',
    label: 'Hablar',
    desc: 'Mantené esta tecla o botón del mouse para dictar. El audio va a donde hiciste click por última vez: una terminal o el chat de LanIde. Doble-tap de la tecla o doble-click del botón: dictado fijado — grabás sin sostener nada y podés moverte por el workspace; la misma tecla o botón envía, Esc cancela.',
...
function _storageKey(id) { return `lanide.control.${id}`; }
...
function _pttMarcarFijado(ctrl) {
  ...
  hint.textContent = window.PttFijado?.hintFijado(
    _renderBindingLabel(_controlBindings[ctrl.id]), window.LanIdeI18n?.t) ?? '';
}
...
// una terminal. Devuelve true si LAN_IDE "consume" la tecla (xterm debería
// ignorarla y no mandarla al PTY).
window._lanideHandleControlKey = function (e) {
  if (_capturando) return false;
  // Atajos del editor cuando una terminal tiene foco (xterm captura el teclado):
  // delegar en LanIdeEditor. Devolver true → xterm no manda la tecla al PTY.
  if (e.type === 'keydown' && (e.metaKey || e.ctrlKey) && e.shiftKey &&
      (e.code === 'KeyF' || e.key === 'F' || e.key === 'f')) {
    if (window.LanIdeEditor) { window.LanIdeEditor.openSearch(); return true; }
  }
  ...
  if (e.type === 'keydown' && (e.metaKey || e.ctrlKey) && !e.altKey &&
    ...
    if (window.LanIdeEditor?.openPalette) {
      window.LanIdeEditor.openPalette(e.shiftKey ? 'comando' : 'archivo');
...
const _VOZ = {
  traducir:   () => localStorage.getItem('lanide.voz.traducir')   === '1',     // dictado → inglés (Whisper translate)
  autoenviar: () => localStorage.getItem('lanide.voz.autoenviar') === '1',     // al soltar, enviar el mensaje a LanIde
  sonido:     () => localStorage.getItem('lanide.voz.sonido')     !== 'off',   // bip al empezar/terminar de escuchar
  pill:       () => localStorage.getItem('lanide.voz.pill')       !== 'off',   // mostrar la píldora idle (tap/linger)
};
...
const _PILL_POS_KEY = 'lanide.voz.pos';
...
function _pttProcesando() {
  ...
  if (tr) tr.textContent = window.LanIdeI18n?.t?.('Transcribiendo…') ?? 'Transcribiendo…';
  el.classList.add('visible');
  _reanchorPill();
  window.LanIdeVoiceField?.procesando?.();   // el campo queda encendido, ya sin seguir la voz
}
...
function _lingerVoicePill(ctrl) {
  _detenerWaveform();
  window.LanIdeVoiceField?.apagar?.();   // el dictado terminó: la ventana se apaga
...
const _VOZ_OPTS = [
  { key: 'traducir',   icon: _VP_TRANS, tit: 'Traducir a inglés',   sub: 'Transcribe tu voz y la pasa a inglés',     ls: 'lanide.voz.traducir',   on: '1',  off: '0' },
  { key: 'sonido',     icon: _VP_SND,   tit: 'Sonido al dictar',    sub: 'Un bip al empezar y al terminar de escuchar', ls: 'lanide.voz.sonido',  on: 'on', off: 'off' },
  { key: 'autoenviar', icon: _VP_SEND,  tit: 'Auto-enviar a LanIde', sub: 'Manda el mensaje al soltar la tecla',      ls: 'lanide.voz.autoenviar', on: '1',  off: '0' },
];
...
function _renderVoicePop(pop) {
  ...
  pop.querySelector('.vp-foot-link')?.addEventListener('click', () => {
    _cerrarVoicePop();
    window.LanIdeSettings?.open?.('voz');
  });
}
...
// window.LanIdeAudioMeter (shared/audio-meter.js, testeada en Node).
...
function _iniciarWaveform(stream) {
  const M = window.LanIdeAudioMeter;
  if (!M || !stream) return;
...
// (ej. LanIde oculto sin terminal activa).
...
// El modal Controles legacy se eliminó (Task 69); la UI vive en LanIdeSettings (Voz).
// _refrescarControlesModal es un no-op que notifica a settings para refrescar su fila.

function _refrescarControlesModal() { window.LanIdeControls?.onCambio?.(); }
...
function _iniciarCaptura(id) {
  ...
  // El botón puede vivir en LanIdeSettings (sección Voz). Si no hay botón visible
