'use strict';
// LAN_IDE — Diagnóstico del freeze post-update (instrumentación temporal del
...
//           (terminal.js suma en window._lanideDiagRx). Si el usuario rueda
...
(function (root) {
  ...
  function arrancar(motivo) {
    ...
    _corriendo = true;
    root._lanideDiagRx = root._lanideDiagRx || 0;   // terminal.js suma acá
    ...
    let _rxPrev = root._lanideDiagRx;
    const _tick = () => {
      const ahora = performance.now();
      const rx = root._lanideDiagRx;
      muestras.push({
  ...
  root.LanIdeDiagUpdate = { arrancar, _pure };
  if (typeof module !== 'undefined' && module.exports) module.exports = root.LanIdeDiagUpdate;
