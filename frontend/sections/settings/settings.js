// LAN_IDE — Configuración · rediseño "Liquid Glass · Datasheet" (2026-07-19).
...
// Expone window.LanIdeSettings = { init, onProjectChanged, open, close, isOpen,
...
(() => {
  ...
  const _t = (s) => window.LanIdeI18n?.t?.(s) ?? s;
  ...
  function _pintarValores() {
    const tema = window.LanIdeThemes?.actual?.() || 'violeta';
    const v = {
      voz: '',
  ...
  function _indexAjustes() {
    ...
    (window.LanIdeControls?.list?.() || []).forEach(c =>
  ...
  function close() {
    ...
    if (window.LanIdeControls) window.LanIdeControls.onCambio = null;
  ...
  /* ═══════════════════════════════════════════════════════════
     ...
     ═══════════════════════════════════════════════════════════ */
  const _DICTADO_OPTS = [
    { ls: 'lanide.voz.traducir',   on: '1',  off: '0',   def: false, label: 'Traducir a inglés',   desc: 'Lo que dictás se transcribe y se pasa a inglés.' },
    { ls: 'lanide.voz.sonido',     on: 'on', off: 'off', def: true,  label: 'Bip al dictar',       desc: 'Un tono corto al empezar y al terminar de escuchar.' },
    { ls: 'lanide.voz.autoenviar', on: '1',  off: '0',   def: false, label: 'Auto-enviar',         desc: 'Manda el mensaje al soltar la tecla, sin apretar ↵.' },
    { ls: 'lanide.voz.pill',       on: 'on', off: 'off', def: true,  label: 'Píldora de voz',      desc: 'La barra compacta que aparece mientras hablás.' },
  ];
  const _dictOn = (o) => { const raw = localStorage.getItem(o.ls); return raw === null ? o.def : raw === o.on; };

  function _renderVoz(b) {
    const ptt = (window.LanIdeControls?.list?.() || []).find(c => c.mode !== 'press');
    const lbl = () => window.LanIdeControls?.label?.(ptt?.id || 'mic-ptt') || 'Alt';
    ...
    const pintarTecla = () => {
      ...
      return `
        <div class="vz">
          ...
          <div class="vz-txt">
            <p><b>${esc(_t('Mantené'))} ${esc(k)}</b> ${esc(_t('para hablar. El audio va a donde hiciste click por última vez: una terminal o el chat de LanIde.'))}</p>
    ...
    const pintar = () => {
      b.innerHTML =
        ...
        blk('avisos', 'cuando no mirás',
          ...
          setRow('Sonido al terminar una tarea',
            ...
            sw('sx-snd', window.LanIdeSonido ? window.LanIdeSonido.get()
                        : localStorage.getItem('lanide.sonidoTareas') !== 'off',
      ...
      const onNotif = localStorage.getItem('lanide.notif.os') === '1' && concedido;
      if (notif) notif.checked = onNotif;
      if (notifDesc && onNotif) notifDesc.textContent = _t('Activadas');
      notif?.addEventListener('change', async () => {
        if (notif.checked) {
          const ok = await (window.LanIdeNotify?.pedirPermiso?.() ?? Promise.resolve(false));
          if (ok) { localStorage.setItem('lanide.notif.os', '1'); if (notifDesc) notifDesc.textContent = _t('Activadas'); }
          else {
            notif.checked = false;
            localStorage.removeItem('lanide.notif.os');
      ...
      // Sonido de eventos de agente (fuente de verdad: window.LanIdeSonido)
      const snd = b.querySelector('#sx-snd');
      snd?.addEventListener('change', () => {
        if (window.LanIdeSonido) window.LanIdeSonido.set(snd.checked);
        else localStorage.setItem('lanide.sonidoTareas', snd.checked ? 'on' : 'off');
      });
      ...
      b.querySelector('.set-keybind')?.addEventListener('click', (e) => {
        if (e.currentTarget.dataset.id === 'mic-ptt' && window.LanIdeGroqSetup?.abrir) {
          window.LanIdeGroqSetup.abrir();
          return;
        }
        window.LanIdeControls?.capturar?.(e.currentTarget.dataset.id);
      });
    };

    window.LanIdeControls && (window.LanIdeControls.onCambio = () => { pintar(); _pintarValores(); });
    pintar();
  }
  ...
  const _ATAJOS_FIJOS = {
    ...
    KeyJ: { mod: 'Ctrl', que: 'Panel → LanIde' },
  ...
  function _bindingCode(id) {
    try {
      const raw = localStorage.getItem(`lanide.control.${id}`);
  ...
  function _renderAtajos(b) {
    const pintar = () => {
      ...
      const ctrls = window.LanIdeControls?.list?.() || [];
      const teclado = `
        <div class="kb">
      ...
      const reasignar = ctrls.map(c => `
        <div class="sx-set" data-key="${esc(c.label)}">
          ...
          <button class="kb-bind settings-keybind set-keybind" data-id="${esc(c.id)}" type="button">
            ...
            <span class="settings-kbd">${esc(window.LanIdeControls.label(c.id))}</span>
          </button>
      ...
      const mouseSolo = ctrls.filter(c => !_bindingCode(c.id))
        .map(c => `<div><span class="kb-chip">${esc(window.LanIdeControls.label(c.id))}</span> ${esc(c.label)} — ${esc(_t('es un botón del mouse, por eso no está en el mapa'))}</div>`).join('');
      ...
      b.querySelectorAll('.kb-k.bound').forEach(k => {
        ...
        k.addEventListener('click', () => {
          ...
          if (id) window.LanIdeControls?.capturar?.(id);
        });
      });
      b.querySelectorAll('.kb-bind').forEach(x =>
        x.addEventListener('click', () => window.LanIdeControls?.capturar?.(x.dataset.id)));
      b.querySelectorAll('.kb-reset').forEach(x =>
        x.addEventListener('click', () => { window.LanIdeControls?.reset?.(x.dataset.id); pintar(); _pintarValores(); }));
    };
  ...
  const _esClaro = (t) => !!window.LanIdeThemes?.esTemaClaro?.(t);
  const _temaActual = () => window.LanIdeThemes?.actual?.() || 'violeta';
  ...
  function _renderApariencia(b) {
    ...
    const temas = ORDEN_TEMAS.filter(t => (window.LanIdeThemes?.TEMAS || ORDEN_TEMAS).includes(t));
    const tinte = window.LanIdeThemes?.tinte?.() || { matiz: 0, saturacion: 100, profundidad: 0 };
    const idioma = window.LanIdeI18n?.lang?.() || 'es';
    ...
    const escAct = window.LanIdeEscala?.actual?.() ?? 100;
    const escMin = window.LanIdeEscala?.MIN ?? 70;
    const escMax = window.LanIdeEscala?.MAX ?? 150;
    const escDef = window.LanIdeEscala?.DEF ?? 100;
          ...
          <span class="sx-slider">
            <span class="ap-esc-track" style="--esc-det:${detente}">
              <input type="range" id="ap-esc" min="${escMin}" max="${escMax}"
                     step="${window.LanIdeEscala?.PASO ?? 5}" value="${escAct}"
    ...
    b.innerHTML =
      ...
      blk('interfaz', 'idioma y modo',
        ...
        setRow('Auto-iniciar el preview móvil',
          ...
          sw('ap-mob', localStorage.getItem('lanide.autoMobilePreview') !== '0', _t('Auto-iniciar el preview móvil')),
    ...
    b.querySelectorAll('.ap-t').forEach(x => {
      ...
      x.addEventListener('click', () => {
        clearTimeout(tPrevia);
        window.LanIdeThemes?.aplicar?.(x.dataset.tema);
    ...
    [h, s, p].forEach(el => {
      ...
      el.addEventListener('input', () => { pintarTon(); window.LanIdeThemes?.setTinte?.(leer(), { silencioso: true }); });
      el.addEventListener('change', () => window.LanIdeThemes?.setTinte?.(leer()));
    });
    reset.addEventListener('click', () => {
      h.value = 0; s.value = 100; p.value = 0;
      pintarTon(); window.LanIdeThemes?.setTinte?.(leer());
    });
    pintarTon();
    ...
    const pintarEsc = () => {
      eslOut.textContent = window.LanIdeEscala?.etiqueta?.(esl.value) ?? `${esl.value}%`;
      eslReset.hidden = Number(esl.value) === (window.LanIdeEscala?.DEF ?? 100);
    };
    esl.addEventListener('input', pintarEsc);
    esl.addEventListener('change', () => { pintarEsc(); window.LanIdeEscala?.aplicar?.(esl.value); });
    eslReset.addEventListener('click', () => {
      esl.value = String(window.LanIdeEscala?.DEF ?? 100);
      pintarEsc(); window.LanIdeEscala?.aplicar?.(esl.value);
    });
    pintarEsc();
    ...
    b.querySelector('#ap-lang').addEventListener('click', (e) => {
      ...
      window.LanIdeI18n?.setLang?.(btn.dataset.l);
    });
    b.querySelector('#ap-mob').addEventListener('change', (e) =>
      localStorage.setItem('lanide.autoMobilePreview', e.target.checked ? '1' : '0'));
  }
  ...
  /* ═══════════════════════════════════════════════════════════
     5 · EXTENSIONES — el markup .ps-* lo cablea LanIdeSkills.montar()
  ...
  function _renderSkills(b) {
    ...
    window.LanIdeSkills?.montar?.();   // bridge expuesto desde workspace.js
    _wireSkillsBuscar(b);
  }
  ...
  function _pintarMemoria(b, sal, mem) {
    ...
    const lista = recientes.length ? `<div class="mm-list">${recientes.map(m => `
      ...
      : `<div class="sx-empty"><b>${esc(_t('Todavía no hay memorias'))}</b>
          <p>${esc(_t('Cuando un agente descubre algo que el resto debería saber, lo deja acá y el recall se lo inyecta al siguiente. Se guardan en .lan-ide/memory/.'))}</p></div>`;
    ...
    const abrir = () => window.LanIdeMemory?.abrir?.();
