'use strict';
// ═══════════════════════════════════════════════════════════════════════════
// LAN_IDE · Radio — Fuente V2: Spotify (Web Playback SDK)
// window.LanIdeRadioSpotify
//
...
// ['lanide.spotify.autorizado'] y se decora con toast. El botón "Conectá
...
// LanIdeRadio.registrarFuente() al cargar (con reitero por si el API llega
// tarde). Si el API aún no existe, silencioso.
// ═══════════════════════════════════════════════════════════════════════════
(function (root) {
  if (root.LanIdeRadioSpotify) return;

  const ID = 'spotify';
  const KEY_AUTH = 'lanide.spotify.autorizado';
...
.jr-sp-x svg{width:14px;height:14px}`;
  ...
  function _syncSetup() {
    const pop = document.getElementById('lanide-radio-pop');
    if (!pop) return;
  ...
  function _abrirVentanaAuth(url) {
    ...
    try { _authWin = window.open(url, 'lanide-spotify-auth', 'width=560,height=740,popup=1'); } catch {}
  ...
  // ── registro (radio.js: LanIdeRadio.registrarFuente) ──
