'use strict';
// ═══════════════════════════════════════════════════════════════════════════
// LAN_IDE · Radio GLOBAL (window.LanIdeRadio)
...
// registra SU player y SUS endpoints vía window.LanIdeRadio.registrarFuente()
...
// ═══════════════════════════════════════════════════════════════════════════
(function (root) {
  if (root.LanIdeRadio) return;

  const KEY = 'lanide.preview.radio';           // estado de la pista (reanudación)
  const SRCKEY = 'lanide.preview.radio.src';    // fuente activa del buscador (persistida)
  ...
  const _t = (s) => (root.LanIdeI18n && root.LanIdeI18n.t) ? root.LanIdeI18n.t(s) : s;
  ...
  // agnóstico. Registrar la tuya con window.LanIdeRadio.registrarFuente({...})
  ...
  function _asegurarAudio() {
    ...
    f.className = 'jr-audio'; f.id = 'lanide-radio-audio'; f.title = 'Radio (audio)';
  ...
  function _saludar() {
    try {
      _audio?.contentWindow?.postMessage(JSON.stringify({ event: 'listening', id: 'lanide-radio', channel: 'widget' }), 'https://www.youtube.com');
  ...
  function _asegurarAudioLocal() {
    ...
    a.id = 'lanide-radio-audio-local'; a.preload = 'metadata';
  ...
  function pauseForTwitch() { _twitch++; if (_twitch === 1 && _state && _state.track && _state.sonando) { _cmd('pauseVideo'); const m = $('#lanide-radio-mini'); if (m) m.classList.add('twitch'); } }
  function resumeAfterTwitch() { if (_twitch > 0) _twitch--; if (_twitch === 0) { const m = $('#lanide-radio-mini'); if (m) m.classList.remove('twitch'); if (_state && _state.track && _state.sonando) _cmd('playVideo'); } }
  ...
  const VOLKEY = 'lanide.preview.radio.vol';
  function _volLoad() {
  ...
  // observer de LanIdeI18n las traduzca solo al re-render y al cambiar de idioma.
  let _i18nDone = false;
  function _i18n() {
    if (_i18nDone || !(root.LanIdeI18n && root.LanIdeI18n.agregar)) return;
    _i18nDone = true;
  ...
  function _montar() {
    ...
    mini.className = 'jr-mini'; mini.id = 'lanide-radio-mini'; mini.title = 'Radio';
    ...
    const pop = document.createElement('div'); pop.className = 'jr-pop'; pop.id = 'lanide-radio-pop';
    pop.innerHTML =
    ...
    const catcher = document.createElement('div'); catcher.className = 'jr-catch'; catcher.id = 'lanide-radio-catch';
    ...
    window.addEventListener('lanide:lang', () => { _syncPlaceholder(); _renderHints(); });
  ...
  function _toggle(force) {
    ...
    $('#lanide-radio-pop').classList.toggle('on', _open);
    $('#lanide-radio-catch').classList.toggle('on', _open);
    $('#lanide-radio-mini').classList.toggle('abierto', _open);
  ...
  function _alinear() {
    const m = $('#lanide-radio-mini'), pop = $('#lanide-radio-pop'); if (!m || !pop) return;
  ...
  function _renderMini() {
    const m = $('#lanide-radio-mini'); if (!m) return;
  ...
  // configurado", sesión) los empuja la fuente vía LanIdeRadio.hintDeFuente()
  ...
  function _renderHints() {
    ...
    if (root.LanIdeI18n && typeof root.LanIdeI18n.aplicar === 'function') {
      try { root.LanIdeI18n.aplicar(cont); } catch {}
    }
  }
  ...
  // window.LanIdeRadio.registrarFuente({
  ...
  root.LanIdeRadio = { init, open, close, play: adopt, adopt, pauseForTwitch, resumeAfterTwitch, estado: () => _state, registrarFuente, fuentes: () => _ordenFuentes.slice(), hintDeFuente, fuenteHint: hintDeFuente };
