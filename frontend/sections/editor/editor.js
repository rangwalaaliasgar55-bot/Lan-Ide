// LAN_IDE — LanIdeEditor: panel editor (file tree + Monaco), extraído de workspace.js
//
// Script CLÁSICO, cargado DESPUÉS de workspace.js. Espeja el patrón de
// OrchestratorPanel/window.lanidePanel: posee TODO el estado del editor y
// expone una superficie pública CONGELADA en window.LanIdeEditor que el resto
...
(function () {
  ...
  //
  // Modelos Monaco compartidos por URI inmemory://lanide/<path>: ambos
  ...
  function _ftUri(path) {
    return monaco.Uri.parse('inmemory://lanide/' + path);
  }
  ...
  const _LE = (es, en) => (window.LanIdeI18n?.lang?.() === 'en' ? en : es);
  ...
  function _cpEjecutar(item) {
    ...
    else LanIdeEditor.abrirArchivo(item.path);
  }
  ...
  function _ftMostrarMenu(x, y, node) {
    ...
    const zx = window.LanIdeEscala?.aCss?.(x) ?? x;
    const zy = window.LanIdeEscala?.aCss?.(y) ?? y;
    ...
    if (r.right > window.innerWidth)  menu.style.left = (zx - (window.LanIdeEscala?.aCss?.(r.width) ?? r.width))  + 'px';
    if (r.bottom > window.innerHeight) menu.style.top = (zy - (window.LanIdeEscala?.aCss?.(r.height) ?? r.height)) + 'px';
  ...
  async function _ftCrearInline(parentDir, tipo) {
    ...
    try {
      ...
      await LanIdeEditor.refrescarArbol();
      if (tipo === 'file') LanIdeEditor.abrirArchivo(path);
    } catch (err) {
  ...
  function _ftRemapearTab(viejo, nuevo) {
    ...
    const uriVieja = monaco.Uri.parse('inmemory://lanide/' + viejo);
    monaco.editor.getModel(uriVieja)?.dispose();
    const uriNueva = monaco.Uri.parse('inmemory://lanide/' + nuevo);
  ...
  async function _ftRenombrarInline(node) {
    ...
    try {
      ...
      if (tipo === 'dir') {
        ...
        if (LanIdeEditor.estaAbierto(src)) {
  ...
  window.addEventListener('lanide:lang', () => {
  ...
  function cargarMonacoLazy() {
    ...
    _monacoPromise = (async () => {
      try {
        ...
        monaco.editor.defineTheme('lanide-dark', _temaMonacoDesdeTokens());
  ...
  // Monaco inmemory://lanide/* y vacía el LRU. Robusto si Monaco aún no cargó
  ...
  function _resetEditorState() {
    ...
    // Disponer TODOS los modelos inmemory://lanide/* (sobreviven entre proyectos).
    try {
      if (window.monaco?.editor?.getModels) {
        for (const m of monaco.editor.getModels()) {
          const uri = m.uri?.toString?.() || '';
          if (uri.startsWith('inmemory://lanide/')) m.dispose();
        }
      }
  ...
  async function _abrirArchivoEnLinea(path, linea, col) {
    ...
    if (!rel) return false;
    try { window.LanIdeDock?.open?.('editor'); } catch (_) {}
  ...
  function _verDiferencias(g, tab, path) {
    ...
    const de = monaco.editor.createDiffEditor(document.getElementById('ed-diff-host'), {
      readOnly: true, automaticLayout: true, theme: 'lanide-dark',
  ...
  function _sesClave() { return _projectId ? `lanide.editor.sesion.${_projectId}` : null; }
  ...
  // ═══════════════════════════════════════════════════════════════════
  //  Superficie pública CONGELADA — window.LanIdeEditor
  ...
  window.LanIdeEditor = {
    ...
    // Toggle del panel (lo invoca el shim window.togglePanel → LanIdeDock)
    ...
    // Relayout (lo invoca LanIdeDock al redimensionar/maximizar): ambos grupos.
    relayout()          { _relayout(); },
