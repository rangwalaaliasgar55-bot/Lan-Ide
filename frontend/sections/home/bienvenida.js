 ...
 * POR QUÉ EXISTE
 * ==============
 * LanIde daba por hecho que los CLIs estaban: abría la terminal y tipeaba
...
(function (global) {
  ...
  function _construir() {
    ...
    _el.innerHTML = `
      ...
      <p class="bv-texto">
        LanIde los orquesta; la inferencia corre en <b>tus</b> cuentas.
  ...
  async function _instalar(boton) {
    ...
    try {
      ...
      boton.disabled = false;
      boton.textContent = 'Reintentar';
      if (global.toast) toast((window.LanIdeI18n.t('No se pudo instalar: {msg}') || 'No se pudo instalar: {msg}').replace('{msg}', e.message), 'error');
    }
  }
