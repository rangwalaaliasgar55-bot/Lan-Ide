// LAN_IDE — Gestión de instancias xterm.js y WebSocket por terminal
...
const _termT = (s) => (window.LanIdeI18n && window.LanIdeI18n.t) ? window.LanIdeI18n.t(s) : s;
...
// de los agentes — skill qa-browser-lanide) mira las terminales SIN tocar nada:
...
// window._lanideNudgeOff = 1. Ver terminal-nudge.js + [[negro-fullscreen-frames-2026]].
(function _vigilanteTranscriptBlanco() {
  ...
  setInterval(() => {
    if (ES_OBSERVADOR || window._lanideNudgeOff) return;
...
// editor de LanIde en esa línea (no el browser externo). Sólo extensiones de
      ...
      provideLinks(bufferLineNumber, callback) {
        ...
        while ((m = RE_ARCHIVO_LINEA.exec(text))) {
          ...
          links.push({
            ...
            text: full,
            activate: () => {
              try { window.LanIdeEditor?.abrirArchivoEnLinea?.(rel, ln, col); } catch (_) {}
            },
          });
        }
...
function crearTerminal(containerId, terminalId, tipoIa = 'manual', intentoAuto = 0) {
  ...
  // Los links LOCALES (dev server del agente, demo /static de LanIde) se abren EN el
  ...
  // `archivo:línea` de la salida de los agentes → editor de LanIde en esa línea.
  ...
  term.attachCustomKeyEventHandler((e) => {
    if (window._lanideHandleControlKey?.(e)) return false;
  ...
  container.addEventListener('wheel', e => {
    if (e._lanideRuedaSint) return;               // copia sintética nuestra: dejarla llegar a xterm
  ...
  function _mostrarDrop(clase) {
    ...
    if (container.dataset.drop !== clase) {          // sólo re-pintar al cambiar de clase
      ...
      const tr = s => (window.LanIdeI18n ? window.LanIdeI18n.t(s) : s);
  ...
  function _abrirWS() {
    ...
    ws.onmessage = e => {
      ...
      if (window._lanideDiagRx != null) window._lanideDiagRx += (e.data && e.data.length) || 0;
      _quitarSkeleton();
    ...
    ws.onclose = (ev) => {
      ...
      const dec = window.TerminalFlow?.decidirReintento?.({
        ...
        recargando: !!window.LanIdeUpdater?.recargando, autoIntento: _autoIntento,
      ...
      if (dec === 'programar') {
        ...
        setTimeout(() => {
          ...
          if (window.LanIdeUpdater?.recargando) return;
 ...
 * runtime: window._lanideNudgeOff = 1 (mismo criterio que cortarFrameSync).
 */
...
// devuelve su ruta absoluta en disco (/tmp/lanide_uploads/...). El browser no
...
function _mostrarMenuContextual(x, y, term, ws, copiar) {
  ...
  const z = window.LanIdeEscala?.zoom?.() || 1;
