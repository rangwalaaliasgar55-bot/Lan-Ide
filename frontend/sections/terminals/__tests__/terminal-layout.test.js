...
(async () => {
  ...
  // el doble de ancha. Se observa por el _free persistido en el localStorage fake.
  {
    ...
    await sleep(300);
    const leer = () => JSON.parse(globalThis.localStorage.getItem('lanide.terminals.layout.99')).free;
    const antes = leer();
  ...
  // real (agregarVerticalF) y se observa el _free persistido.
  {
    ...
    const leer = () => JSON.parse(globalThis.localStorage.getItem('lanide.terminals.layout.77')).free;
  ...
  // arriba con 7 dejaba [5,1] (6 terminales en 2 filas) en vez de colapsar a [6].
  {
    TL.setInteracting(false);
    TL.init(88, null);
    const leer = () => JSON.parse(globalThis.localStorage.getItem('lanide.terminals.layout.88')).free;
  ...
  // remove() con >6 mantiene 2 filas canónicas (abajo 3) hasta colapsar en 6
  {
    TL.setInteracting(false);
    TL.init(89, null);
    const leer = () => JSON.parse(globalThis.localStorage.getItem('lanide.terminals.layout.89')).free;
