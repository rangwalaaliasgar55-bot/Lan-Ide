/* ════════════════════════════════════════════════════════════════
   ...
   (así este src relativo resuelve contra LanIde). Corre en la app VISIBLE de
   ...
   2. El <base> hacia Metro hace que expo-router pase URLs de :8081 a
      history.* (SecurityError) y que el HMR intente WS contra LanIde (403):
   ...
   4. FUENTES: el browser exige CORS para @font-face cross-origin y Metro no
      ...
      con @font-face hacia el proxy same-origin /asset de LanIde.
...
(() => {
  ...
  // que lo cargado ES el espejo instrumentado (y no una página de LanIde).
  window.__mpsSampler = true;
  // Origen de LanIde: el src de ESTE script (resuelto antes del <base>).
  let ORIGEN = '*';
  ...
  // cliente de Metro arma ws:// contra el origen de LanIde (fallaba 403 y el
  ...
  // Metro se reescriben al proxy /asset de LanIde (CORS resuelto). Idempotente:
  ...
  function reescribirUrlsCss(txt) {
    ...
    return txt.replace(URL_CSS_RE, (todo, q, u) => {
      ...
      // ABSOLUTA hacia LanIde: una url relativa resolvería contra el <base>
  ...
  // ── Parche 5: los FULL RELOADS no escapan a LanIde. El parche 3 dejó la URL
  ...
  // location.reload() de la app) recarga ESA ruta contra el origen de LanIde →
