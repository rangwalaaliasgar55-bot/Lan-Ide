...
(() => {
  ...
  try {
    S.guias = localStorage.getItem('lanide.mps.guias') === '1';
    S.encaje = localStorage.getItem('lanide.mps.encaje') !== '0';   // default ON
  } catch { /* privado */ }
  ...
  function _wireBar() {
    ...
    menu.addEventListener('click', (e) => {
      ...
      if (act === 'add') _addPhone();
        ...
        S.guias = !S.guias;
        try { localStorage.setItem('lanide.mps.guias', S.guias ? '1' : ''); } catch { /* privado */ }
        _renderPhones(); _syncChecks();
      }
      else if (act === 'encaje') {
        S.encaje = !S.encaje;
        try { localStorage.setItem('lanide.mps.encaje', S.encaje ? '1' : '0'); } catch { /* privado */ }
        _renderPhones(); _syncChecks();
      }
  ...
  function _onPan(e) {
    ...
    const _ze = (window.LanIdeEscala && window.LanIdeEscala.zoom) ? window.LanIdeEscala.zoom() : 1;
  ...
  function _ensurePhone(p) {
    ...
    // LanIde SE CARGÓ adentro): el sampler se marca con window.__mpsSampler —
  ...
  const _lsWebs = () => `lanide_mps_webs_${_projectId}`;
  ...
  function _cargarWeb(w, node) {
    ...
    // Browser normal: iframe best-effort (sitios que bloquean → solo en LanIde).
  ...
  // claves de EAS, comandos, pendientes). Se guardan en la DB LOCAL de LanIde
  // (data/lanide.db, gitignoreada) vía /api/mobile-preview/{pid}/notas — nunca
  ...
  const _lsSinTel = () => `lanide_mps_sin_tel_${_projectId}`;
  ...
  // navega esa ruta contra LanIde → el dashboard adentro del teléfono. El
  ...
  async function _detectarUnaVez() {
    ...
    if (d && d.web && d.url) {
      ...
      if (nuevo) {
        ...
        if (window.LanIdeDock && window.LanIdeDock.activeTab && window.LanIdeDock.activeTab() !== 'mobile') {
          window.LanIdeDock.notify && window.LanIdeDock.notify('mobile', 1);
        }
      }
    } else if (d && d.nativo) {
