// LAN_IDE — LanIdeDock: el Panel Único dockeado a la derecha del mosaico.
// Header de pestañas (solo iconos) + splitter contra las terminales + maximizar.
// Espeja el patrón window.LanIdeEditor / window.TerminalLayout: lógica pura
...
(function (global) {
  ...
  // 'lanide' incluido: el orquestador se despliega a pantalla completa (su
  // layout es editorial-responsive por container-query) además de vivir
  // angosto en el dock como panel lateral.
  const MAXIMIZABLE = new Set(['preview', 'lanide']);   // 'editor' retirado del dock (→ editor deslizante)
  ...
  global.LanIdeDock = Object.assign(global.LanIdeDock || {}, { _pure: pure });
  ...
  if (typeof document !== 'undefined') {
    ...
    let _state = {
      ...
      // un lanide.dock.tab persistido en 'web' degrada a DEFAULT_TAB.
      // editor: false — el editor Monaco salió del dock; lo reemplaza el editor
      // deslizante (#jw-editor / window.LanIdeSlideEditor). Con false, un
      // lanide.dock.tab persistido en 'editor' degrada a DEFAULT_TAB.
      badges: {}, visible: { mobile: false, preview: true, lanide: true,
    ...
    const _kOpen  = () => `lanide.dock.open.${_projectId}`;
    const _kTab   = () => `lanide.dock.tab.${_projectId}`;
    ...
    const _kMVis  = () => `lanide.dock.mvis.${_projectId}`;
    const K_WIDTH = 'lanide.dock.width';   // legado: fallback para proyectos sin ancho propio
    ...
    const _kWidth = () => `lanide.dock.width.${_projectId}`;
    ...
    const _kAgentW = (id) => `lanide.dock.agentw.${id}`;
    ...
    const _kAgentTab = (id) => `lanide.dock.agenttab.${id}`;
    ...
    const _kAgentOpen = (id) => `lanide.dock.agentopen.${id}`;
    // (La franja usa 'lanide.strip.hidden' pero la maneja la Fase 1; el dock no.)
    ...
    function _restore() {
      ...
      // setProject) NO leer 'lanide.dock.*.null' — el width sí es global y ya cargó.
    ...
    function _render(prev) {
      ...
      if (cambioMax && _state.open) {
        if (_state.tab === 'editor') {
          requestAnimationFrame(() => global.LanIdeEditor?.relayout?.());
          setTimeout(() => global.LanIdeEditor?.relayout?.(), 240);
        }
        // preview (iframe): no requiere refit (spec §4.1).
      }
    }

    // _El toggle de la franja es de la Fase 1 — LanIdeDock no lo toca._
    // (La franja, su botón #jw-strip-hide, el atajo Ctrl+B, la persistencia
    //  'lanide.strip.hidden' y la clase body.jw-strip-off los implementa
    ...
    function _onSplitterMove(e) {
      ...
      const dx = global.LanIdeEscala?.aCss?.(_rz.startX - e.clientX) ?? (_rz.startX - e.clientX);
    ...
    function _onSplitterUp() {
      ...
      global.TerminalLayout?.relayoutAll?.();
      _cb.onResized?.();
      if (_state.tab === 'editor') global.LanIdeEditor?.relayout?.();
    }
    ...
    function _wire() {
      ...
      //  LanIdeDock no cablea esos controles.)
      // Splitter
    ...
    Object.assign(global.LanIdeDock = global.LanIdeDock || {}, {
      _pure: pure,
      init(cb) {
        _cb = cb || {};
        _wire();
