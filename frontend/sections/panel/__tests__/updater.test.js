// Tests de la lógica pura de LanIdeUpdater (updater.js).
'use strict';
...
const { debeMostrar, etiquetaBanner, lineaVersion, esHotfix, etiquetaVersionNueva,
        ...
        decidirOverlayPostReload, tilesOverlay } = globalThis.LanIdeUpdater._pure;
...
// sin transición conocida cae a "LanIde vX.Y.Z"; vacío si no hay versión
...
assert.strictEqual(lineaVersion({ corriendo: '1.5.08', proxima: '1.5.08' }), 'LanIde v1.5.08');
assert.strictEqual(lineaVersion({ disponible: '1.5.1' }), 'LanIde v1.5.1');   // server viejo sin 'proxima'
...
assert.strictEqual(lineaAlDia({ corriendo: '1.7.00' }), 'LanIde v1.7');
...
// aunque otra terminal de LanIde siga trabajando (pedido del usuario 2026-06-18).
...
assert.strictEqual(lineaAlDia({ corriendo: '1.5.1' }), 'LanIde v1.5.1');
...
// reinician el server — el reinicio lo dispara el usuario o LanIde mismo)
const t = textosCaida();
...
// (debeOfrecerSalida y el botón "Seguir usando LanIde" se eliminaron el
