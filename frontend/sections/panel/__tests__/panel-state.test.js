...
// ── open / close / toggle ─────────────────────────────────────────
{
  ...
  s = nextState(s, { type: 'open', tab: 'lanide' });
  assert.strictEqual(s.tab, 'lanide', 'open con tab cambia de pestaña');
...
// ── setTab limpia el badge de la pestaña abierta ──────────────────
{
  let s = base();
  s.open = true;
  s.badges = { lanide: 3, tasks: 2 };
  s = nextState(s, { type: 'setTab', tab: 'lanide' });
  assert.strictEqual(s.tab, 'lanide', 'setTab cambia la pestaña');
  assert.strictEqual(s.badges.lanide, 0, 'setTab limpia el badge de la pestaña abierta');
...
{
  let s = base(); s.open = true; s.tab = 'lanide';
  // pestaña activa + dock abierto → NO suma
  s = nextState(s, { type: 'notify', tab: 'lanide', n: 1 });
  assert.strictEqual(s.badges.lanide || 0, 0, 'notify NO suma a la pestaña activa visible');
  ...
  let c = base(); c.open = false; c.tab = 'lanide';
  c = nextState(c, { type: 'notify', tab: 'lanide', n: 1 });
  assert.strictEqual(c.badges.lanide, 1, 'notify suma a la tab actual si el dock está cerrado');
...
// ── badgeTotal: suma de los badges de pestañas ────────────────────
{
  ...
  assert.strictEqual(badgeTotal({ lanide: 3, tasks: 2, review: 0 }), 5, 'badgeTotal suma');
...
// ── maximize: válido en preview|lanide ────────────────────────────
{
  ...
  // lanide (orquestador) ahora es maximizable → pantalla completa
  let j = base(); j.open = true; j.tab = 'lanide';
  j = nextState(j, { type: 'setMax', value: true });
  assert.strictEqual(j.maximized, true, 'maximize permitido en lanide (orquestador)');
...
{
  const { restoreTab } = D;
  assert.deepStrictEqual(restoreTab('lanide', { mobile: false }),
    { tab: 'lanide', pending: null }, 'tab visible se restaura directo, sin pendiente');
...
{
  const s0 = base(); s0.open = true; s0.badges = { lanide: 1 };
