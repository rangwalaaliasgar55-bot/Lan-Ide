// LAN_IDE — In-app updater (Sprint 3+): banner "nueva versión" abajo de la franja
...
(function (global) {
  ...
  // conoce (el server bumpea VERSION al aplicar); si no, "LanIde vX.Y.Z".
  function lineaVersion(info) {
    ...
    return v ? `LanIde v${mostrarVersion(v)}` : '';
  }
  ...
  // LanIde siga trabajando; las ediciones sin commitear no lo prenden (pedido del
  // usuario 2026-06-18).
  ...
  // "LanIde v1.5.1" — la versión en la que estás (estado al día).
  function lineaAlDia(info) {
    return (info && info.corriendo) ? `LanIde v${mostrarVersion(info.corriendo)}` : '';
  }
  ...
  global.LanIdeUpdater = Object.assign(global.LanIdeUpdater || {}, { _pure: pure });
  ...
  const _lang = () => { try { return (global.LanIdeI18n && global.LanIdeI18n.lang) ? global.LanIdeI18n.lang() : 'es'; } catch (_) { return 'es'; } };
  ...
  // la versión corriente con un orbe que respira (la "señal de vida" de LanIde);
  ...
  function _renderChip(info) {
    ...
    // La identidad (marca LanIde + wordmark "LanIde") vive al lado en el
  ...
  async function _chequear() {
    try {
      ...
      if (estadoBanner(_info) === 'update') {
        ...
        // (Sin agentes de LanIde trabajando — si los hubiera, el backend no prende
        ...
        // Cara idle: marca LanIde ploteándose + CTA (bilingüe). Cara hover:
  ...
  // usando LanIde" que llamaba acá se ELIMINÓ (pedido del usuario 2026-08-08):
  ...
  function _soltarOverlay() {
    _cerrar();
    global.LanIdeUpdater.recargando = false;
  }
  ...
  function recargarCuandoListo() {
    ...
    global.LanIdeUpdater.recargando = true;
    let intentos = 0;
  ...
  function _overlayPostReload() {
    ...
    try { window.LanIdeDiagUpdate?.arrancar?.('post-update'); } catch (_) {}
  ...
  global.LanIdeUpdater.chequear = _chequear;
  global.LanIdeUpdater.avisarPosibleCaida = _avisarPosibleCaida;
  ...
  global.LanIdeUpdater.recargarCuandoListo = recargarCuandoListo;
  ...
  function _init() {
    window.addEventListener('lanide:lang', _onLang);
