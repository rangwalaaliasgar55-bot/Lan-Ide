...
The single right dock (`window.LanIdeDock`) and the side project strip.

- **Files:**
  - `panel.js` — dock engine: tabs, splitter, maximize, badges, persistence.
    Exposes `window.LanIdeDock` with `open/close/setTab/notify/isOpen/activeTab`
  ...
  - `launcher-state.js` — **pure** logic of the New-project modal.
    Exposes `window.LanIdeLauncherState` with `CLI_LABELS`, `MAX_TERMINALES`,
...
counter/validation logic lives in `launcher-state.js` (`LanIdeLauncherState`); the render and
