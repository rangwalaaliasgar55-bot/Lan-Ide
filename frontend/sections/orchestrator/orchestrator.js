// LAN_IDE — OrchestratorPanel (panel izquierdo premium)
...
const _orchT = (s) => (window.LanIdeI18n && window.LanIdeI18n.t) ? window.LanIdeI18n.t(s) : s;
...
class OrchestratorPanel {
  ...
  _build() {
    ...
    this.el.setAttribute('aria-label', 'Panel LanIde');
    this.el.dataset.state = this.sphereState;

    this.el.innerHTML = `
      <!-- CONSTELACIÓN (fondo vivo — LanIde es el nodo central) -->
      ...
      <footer class="orch-composer">
        ...
        <div class="orch-input-box">
          ...
          <textarea
            ...
            rows="1"
            aria-label="Mensaje para LanIde"
            autocomplete="off"
            spellcheck="true"
          ></textarea>
  ...
  setSphereState(state) {
    ...
    // E2: typing indicator visible solo cuando LanIde está "produciendo" output
  ...
  // Hero editorial ⇄ estado de voz. Con el hero a la vista, mientras LanIde
  ...
  _showTyping() {
    ...
    el.dataset.role = 'lanide';
    el.setAttribute('aria-label', 'LanIde está escribiendo');
    el.innerHTML = `
      <div class="orch-msg-head">
        <div class="orch-avatar orch-avatar-lanide" aria-hidden="true">J</div>
        <div class="orch-msg-meta">
          <span class="orch-msg-author">LanIde</span>
  ...
  addMessage(msg) {
    // Si LanIde manda un mensaje real, el typing indicator deja de tener sentido
    if (msg.role === 'lanide') { this._hideTyping(); }
  ...
  _isWelcomeMsg(m) {
    if (!m || m.role !== 'lanide') return false;
  ...
  /* ══════════════════════════════════════════════════════════
     CONSTELACIÓN — LanIde es el nodo central de una red viva.
  ...
  _renderMessages() {
    ...
    if (this._isHeroState()) {
      ...
      // = LanIde late detrás). Quick actions cablean a onQuickReply.
      ...
      empty.innerHTML = `
        ...
        <!-- Estado de voz: reemplaza al saludo mientras LanIde escucha/piensa (concepto d4) -->
  ...
  _appendMessage(msg, idx = 0) {
    ...
    art.dataset.role = msg.role === 'lanide' ? 'lanide' : 'user';
  ...
  _buildMsgHTML(msg) {
    ...
    const isJarv  = msg.role === 'lanide';
    const avClass = isJarv ? 'orch-avatar-lanide' : 'orch-avatar-user';
    ...
    // Avatar:
    //   - LanIde → mark "J" Instrument Serif italic (coherente con sidebar/home brand)
...
/* ══════════════════════════════════════════════════════════════
   ...
   Reemplaza el panel-left y expone window.lanidePanel
══════════════════════════════════════════════════════════════ */
