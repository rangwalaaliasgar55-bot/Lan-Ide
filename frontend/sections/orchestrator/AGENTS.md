...
- **Public global:** `window.lanidePanel` (the panel instance). The shell adds
  messages and changes state via the instance directly
  (`lanidePanel.addMessage(...)` / `lanidePanel.setSphereState(state)`).
...
LanIde is the **central node of a living neural network** that reacts to voice.
...
- **Header**: the **LanIde** brand (orb + serif italic) is centered and prominent on TOP. No idle
...
- **Composer** (ELITE redesign — no focus rectangle): bar INTEGRATED to the panel surface,
  ...
  shared memory notes (`.lan-ide/memory/`) for the background.
...
- **Editorial chat** (fewer boxes): **LanIde speaks WITHOUT a bubble-box**
...
- **Maximizable**: `panel.js` includes `'lanide'` in `MAXIMIZABLE` → the ⤢ "Fullscreen"
  dock action appears on the LanIde tab and expands it to `fixed inset:0`. The constellation
