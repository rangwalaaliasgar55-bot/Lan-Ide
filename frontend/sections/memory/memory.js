// LAN_IDE — Memoria compartida del proyecto (UI).
...
// Los agentes escriben las memorias como archivos (.lan-ide/memory/) según el
// protocolo inyectado en CLAUDE.md; esta UI es la ventana del humano.
// Expone window.LanIdeMemory = { init, onProjectChanged, abrir }.
...
(() => {
  ...
  let _query     = '';
  let _liveEstado = null;          // estado de LanIdeLiveState
  ...
  function _saludHTML() {
    const meta = window.LanIdeMemoryMeta;
    ...
    let html = '';
    if (probs.length) {
      html += `<div class="mem-salud" title="Salud de .lan-ide/memory/ — links rotos, citas a archivos borrados, huérfanas, contrato de admisión, choques lápida-vs-vigente, cuarentena por no-uso, duplicadas (misma memoria escrita dos veces), lecciones violadas N veces (candidatas a guard) y lecciones que merecen ser globales">
  ...
  const _badgesHTML = (m) =>
    (window.LanIdeMemoryMeta ? LanIdeMemoryMeta.badges(m) : [])
  ...
  function _renderBody() {
    ...
    document.getElementById('mem-count').textContent =
      _t('{n} memoria(s) · .lan-ide/memory/').replace('{n}', _memorias.length);
  ...
  function _renderItems() {
    ...
    cont.innerHTML = filtradas.map((m, i) => `
      <button class="mem-item ${m.slug === _slugAbierta ? 'activo' : ''} ${(m.estado && m.estado !== 'vigente') ? 'mem-item-apagada' : ''}" data-slug="${esc(m.slug)}" style="--i:${Math.min(i, 12)}">
        ...
        <span class="mem-item-sub">${esc(window.LanIdeMemoryMeta ? LanIdeMemoryMeta.subLinea(m) : (m.autor || '—'))}</span>
      </button>`).join('');
  ...
  async function _cargarLive() {
    try {
      ...
      if (!r.ok) return;
      _liveEstado = window.LanIdeLiveState.aplicarSnapshot(
        _liveEstado || window.LanIdeLiveState.crearEstado(), await r.json(), Date.now());
    } catch { /* red */ }
  }
  ...
  function onLiveEvent(data) {
    ...
    _liveEstado = window.LanIdeLiveState.aplicarSnapshot(
      _liveEstado || window.LanIdeLiveState.crearEstado(), data.snapshot, Date.now());
  ...
  async function _renderLive(body, sinFetch) {
    ...
    const L = window.LanIdeLiveState;
  ...
  window.LanIdeMemory = {
    init(projectId) {
      ...
      // El disparador ahora vive en LanIdeSettings (sección Memoria) — el
