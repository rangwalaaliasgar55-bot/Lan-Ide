// LAN_IDE — Task Board (kanban conectado a los workflows del orquestador).
...
//    de lo que LAN_IDE está orquestando.
// Expone window.LanIdeTasks = { init, show, onProjectChanged,
...
(() => {
  ...
  function _render() {
    ...
    for (const c of COLS) {
      ...
      if (!items.length) {
        cont.innerHTML = `<div class="tk-col-empty">${c.id === 'backlog' ? 'Creá una tarea o pedile un workflow a LAN_IDE' : '—'}</div>`;
        continue;
      }
